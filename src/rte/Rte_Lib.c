/** @file         Rte_Lib.c
  ...
  *               RTE_ARPATCH                            = 3
  *
  *               RTE_MSITABLE_CONST_START_MAK           = RTE_START_SEC_CONST_UNSPECIFIED
  *               RTE_MSITABLE_CONST_STOP_MAK            = RTE_STOP_SEC_CONST_UNSPECIFIED
  *
  */
...
#include "Rte_MemMap.h"
extern CONST(Rte_MSITableEntry, RTE_CONST) Rte_MSITable[];
...
FUNC(boolean, RTE_CODE)
Rte_MSITest( Rte_MSICounterType         count,
             Rte_MSIPendingFlagRefType  pendingFlag,
             Rte_MSICounterType         MSIMax )
{
   if ( ( *pendingFlag == FALSE ) && ( count >= MSIMax ) )
   {
      return TRUE;
   }
...
Rte_MainFunction(void)
{
    ...
    // uint16 i;
   // Rte_MSITableEntryRef msi = &Rte_MSITable[0];

   // for ( i = 0; i < RTE_MSITABLE_SIZE; i++ )
   // {
      // if ( (msi->counter ) && msi->incCounter && ( *msi->counter < msi->MSIInit ) )
      // {
// #if defined(OSENV_RTAOSEK)
         // OS_ATOMIC((*msi->counter)++);
// #else
...
// #endif
      ...
      // if ( ( *msi->counter >= msi->MSIInit ) && ( *msi->pending ) )
      // {
         // TaskType t;

         // *msi->pending = FALSE;

         // t = RTE_TASK_FROM_REF( msi->osTask );

         // /* [$Satisfies $SWS 3520] [$Satisfies $SWS40 7526]*/
         // if ( msi->acnt )
         // {
// #if defined(OSENV_RTAOSEK)
            // OS_ATOMIC((*msi->acnt) = TRUE);
// #else
            // Rte_SuspendOSInterrupts();
            // (*msi->acnt) = TRUE;
            // Rte_ResumeOSInterrupts();
...
// #if defined(RTE_MSI_USES_EVENTS)
         // if ( msi->osEvent )
         // {
// #if defined(RTE_VFB_TRACE)
// #if (RTE_VFB_TRACE != 0)
            // Rte_Task_SetEvent( t, RTE_EVENT_FROM_REF( msi->osEvent ) ); /* [$Satisfies $SWS40 5091] */
// #endif /* (RTE_VFB_TRACE != 0) */
// #endif /* defined(RTE_VFB_TRACE)  */
            // (void)SetEvent( t, RTE_EVENT_FROM_REF( msi->osEvent ) );
         // }
         // else
// #endif /* (RTE_MSI_USES_EVENTS != 0) */
         // {
// #if defined(RTE_VFB_TRACE)
// #if (RTE_VFB_TRACE != 0)
            // Rte_Task_Activate( t ); /* [$Satisfies $SWS40 5091] */
// #endif /* (RTE_VFB_TRACE != 0) */
// #endif /* defined(RTE_VFB_TRACE)  */
            // (void)ActivateTask( t );
         // }
                ...
                Rte_ConstDataPtr   data )
{
   ...
   else
   {
      ...
      switch ( q->notification_type )
      {
         ...
         case RTE_MSI:
         {
            Rte_QRefMSIType msiQ = (Rte_QRefMSIType)q;
            if ( Rte_MSITest( *msiQ->msiCounter, msiQ->msiPending, msiQ->msiLimit ) )
            {
               if ( msiQ->acnt )
               {
#if defined(OSENV_RTAOSEK)
                  OS_ATOMIC((*msiQ->acnt) = TRUE);
#else
                  Rte_SuspendOSInterrupts();
                  (*msiQ->acnt) = TRUE;
                  Rte_ResumeOSInterrupts();
               ...
               if ( RTE_TASK_REF_NO_TASK != msiQ->task )
               {
                  TaskType t = RTE_TASK_FROM_REF( msiQ->task );
#if defined(RTE_MSI_USES_EVENTS)
                  if ( msiQ->mask )
                  {
#if defined(RTE_VFB_TRACE)
#if (RTE_VFB_TRACE != 0)
                     Rte_Task_SetEvent( t, RTE_EVENT_FROM_REF( msiQ->mask ) ); /* [$Satisfies $SWS40 5091] */
#endif /* (RTE_VFB_TRACE != 0) */
#endif /* defined(RTE_VFB_TRACE)  */
                     (void)SetEvent( t, RTE_EVENT_FROM_REF( msiQ->mask ) );
                  }
                  else
#endif /* (RTE_MSI_USES_EVENTS != 0) */
                  {
