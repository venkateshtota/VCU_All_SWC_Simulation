    ...
    if ( TRUE == EcuM_Prv_flgIsModuleInitialised_b  )
    {
        ...
        if    (FALSE == EcuM_Prv_flgShutdownInfoDoNotUpdate_b )
        {
            ...
            if(shutdownCause < ECUM_CFG_NUM_SHUTDOWN_CAUSE)
            {
                ...
                EcuM_Rb_dataShutdownInfo_st.ShutdownCause =EcuM_Prv_dataSelectedShutdownCause_u8;
#endif//ECUM_CFG_NVM_BLOCK_CONFIGURED
...
Std_ReturnType EcuM_GetShutdownTarget(
          ...
          EcuM_ShutdownModeType * shutdownMode)
                        {
    /* Local Variable Declaration */
    EcuM_ShutdownTargetType dataShutdownTarget_u8;
    EcuM_ShutdownModeType dataMode_u16;
        ...
        EcuM_ShutdownInfoType * shutdownCauseInfo)
{
    ...
    if (returnvalue_u8 == E_OK)
    {
        if(FALSE!=EcuM_Prv_flgNvMReadStatus_b)                         /*check if read operation was successful*/
        {
            ...
            shutdownCauseInfo->ShutdownCause = EcuM_Prv_dataShutdownInfo_st.ShutdownCause;
#if (ECUM_STARTUP_DURATION == TRUE)
            shutdownCauseInfo->TimeMeasured = EcuM_Prv_dataShutdownInfo_st.TimeMeasured;
