...
/***************************************************************************************************
 ...
 Description      : Places a request for the POST_RUN state. Requests can be placed by every user made
...

{
    ...
    if(FALSE != EcuM_Prv_IsUserHandlingPossible_b(&User_u16, ECUM_REQUESTPOST_RUN_APIID))
    {
        if(FALSE == EcuM_Prv_userPostRUNReqStatus_ab[User_u16])
        {
            /*The request from the user is registered */
            EcuM_Prv_userPostRUNReqStatus_ab[User_u16] = TRUE;
