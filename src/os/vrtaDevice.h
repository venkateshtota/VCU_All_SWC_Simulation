...
protected:
  ...
  vrtaUInt uVal(const vrtaAction *a, vrtaUInt offset) {
    // Unpack little-endian without strict-aliasing issues
    vrtaUInt ret;
    memcpy(&ret, &a->devEmbeddedData.bVal[offset], sizeof(ret));
    return ret;
  };
  ...
  vrtaUInt uVal(const vrtaEvent *e, vrtaUInt offset) {
    // Unpack little-endian without strict-aliasing issues
    vrtaUInt ret;
    memcpy(&ret, &e->devEmbeddedData.bVal[offset], sizeof(ret));
    return ret;
  };
  ...
  void Set_uVal(vrtaEvent *e, vrtaUInt v, vrtaUInt offset) {
    e->devEventData = 0;
    memcpy(&e->devEmbeddedData.bVal[offset], &v, sizeof(v));
