...
#include "Fee_MemMap.h"
uint8 Fee_LLDetectActiveSector(void)
{
    /* MR12 RULE 8.5 VIOLATION: This is no function declaration but a variable only with an external linkage*/
    Fee_stSecDet_tst Fee_stSecDet_st[FEE_NUM_FLASH_BANKS_AVAILABLE];
    Fee_LLSectorOrder_tst TmpSectorOrder_st[FEE_NUM_FLASH_BANKS_AVAILABLE];
    uint8  xCurrSect_u8 = 0;        /* Sector loop counter */
    uint8  xCntFlashBank_u8 = 0;    /* Loop counter */
    uint8  cnt_u8 = 0;              /* Inner loop counter */
    uint32 SectCntMin_u32 = 0;      /* Lowest sector change counter */
    uint8 xActiveSec_u8 = FEE_NO_SEC_IDX_FOUND;  /* Function return */
    uint8 xPhySecIdx_u8;              /* Physical sector index */
    uint8 xLogSecIdx_u8;            /* Logical sector index */
    uint8  xRepeatSortLoop_u8;

    /* This is a redundant initialization of the automatic structure introduced simply
     * to avoid warnings generated by the cchecker tooling in the following usage of the
     * structure Fee_stSecDet_st.
     */
    for (xCurrSect_u8=0u;xCurrSect_u8<FEE_NUM_FLASH_BANKS_AVAILABLE;xCurrSect_u8++)
    {
        /* Initialize sector info array */
        Fee_stSecDet_st[xCurrSect_u8].ctErasedMarker_u8         = 0u;
        Fee_stSecDet_st[xCurrSect_u8].ctUsedMarker_u8           = 0u;
        Fee_stSecDet_st[xCurrSect_u8].ctFullMarker_u8           = 0u;
        Fee_stSecDet_st[xCurrSect_u8].ctEraseReq_u8             = 0u;
        Fee_stSecDet_st[xCurrSect_u8].ctCloneMarker_u8          = 0u;
        Fee_stSecDet_st[xCurrSect_u8].eraseStartFlagFound_b     = FALSE;
        Fee_stSecDet_st[xCurrSect_u8].SecChngCnt_u32            = FEE_SEC_CHANGE_CNT_UNDEF;
        Fee_stSecDet_st[xCurrSect_u8].xStartAddr_u32            = FEE_MAXUINT32;
    }

    /* Collect all sector header information (Note: the complete structure Fee_stSecDet_st is updated) */
    ...
    for(xCurrSect_u8=0u; xCurrSect_u8<FEE_NUM_FLASH_BANKS; xCurrSect_u8++)
    {
        ...
        /* Check if erase start marker is non blank */
        if(TRUE == Fee_stSecDet_st[xCurrSect_u8].eraseStartFlagFound_b)
        {
            /* Sector was being ersaed, ignore any other markers found and se the sector requested to be erased */
    ...
    for(xCntFlashBank_u8=0; xCntFlashBank_u8<FEE_NUM_FLASH_BANKS; xCntFlashBank_u8++)
    {
        ...
        /* MR12 RULE 1.3 VIOLATION: The variable xCurrSect_u8 will be within limit (<FEE_NUM_FLASH_BANKS) when this instruction is called */
...
void Fee_LLSearchSectors(Fee_stSecDet_tst* Fee_stSecDet_ps)
{
    Fee_MarkerProp_t    xRdMarker;                  /* Marker structure */
    uint32              xTmpMarkerContent_u32;      /* Marker content */
    uint8               xCurrSect_u8 = 0;           /* Current sector index */
    uint16              xCalc_crc_u16;              /* CRC16 variable */
    sint8                xMarkerIdx_s8 = -1;            /* Marker index */
    uint32                xAdrBlnkChk_u32 = FEE_NULL;    /* Blank check address */


    /* Set the number of Retries */
    Fee_RdWrRetries_u8 = FEE_MAX_RD_WR_COMP_RETRIES;

    /* Set initial set state explicitely - this was only added to ease the future analysis
     * The variable is also set to this value before calling this function.
     * By setting it here, the implicite dependency is "broken".
     */
    ...
    while (xCurrSect_u8<FEE_NUM_FLASH_BANKS)
    {
        ...
        switch(Fee_RdWrOrder_st.Fee_LLRdState_en)
        {
            ...
            case FEE_LL_INIT_READ_E:
            {
                ...
                Fee_stSecDet_ps[xCurrSect_u8].eraseStartFlagFound_b   = FALSE;
            ...
            case FEE_LL_BLANK_CHECK_E:
            {
                ...
                if(xMarkerIdx_s8 < FEE_NUM_MARKER_IN_HEADER)
                {
                    ...
                    if(Fls_BlankCheck(xAdrBlnkChk_u32 + (FEE_PAGE_LEN-FEE_PHY_PAGE_LEN), FEE_PHY_PAGE_LEN) != E_NOT_OK)
                    {
                        /* Set the next state */
                ...
                {
                    /* Check for Erase Start Flag at the last marker location */
                    Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_BLANK_CHECK_ERASE_START_FLAG_E;
                }
            }
            break;
                ...
                if (xRdMarker.xPattern == FEE_MARKER_PATTERN)
                {
                    ...
                    if (xCalc_crc_u16 == xRdMarker.xChecksum)
                    {
                        ...
                        switch(xRdMarker.xIdent)
                        {
                            ...
                            /* Start marker is ignored, as it cannot determine the state of the sector */
                            case (uint8)FEE_START_MARKER_ID_E:
                                    /* do nothing. */
                            break;
            ...
            case FEE_LL_BLANK_CHECK_ERASE_START_FLAG_E:
            {
                /* Calculate the blank check address for the next marker (markers will start only at a free page) */
                xAdrBlnkChk_u32 += FEE_PAGE_LEN;

                if(Fls_BlankCheck(xAdrBlnkChk_u32, FEE_PAGE_LEN) != E_NOT_OK)
                {
                    /* Set the next state */
                    Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_BLANK_CHECK_ERASE_START_FLAG_WAIT_E;
                }
                else
                {
                    /* Error handling due to wrong user parameters or a currently active FLS
            ...
            /* Wait for the blank check to be finished */
            case FEE_LL_BLANK_CHECK_ERASE_START_FLAG_WAIT_E:
            {
                /* The next state will be set by the Fee_JobEndNotification () or
                 * Fee_JobErrorNotification() function automatically */
            ...
            case FEE_LL_ERASE_START_FLAG_NOT_PRESENT_E:
            {
                /* Clear the flag that erase start flag was not found */
                Fee_stSecDet_ps[xCurrSect_u8].eraseStartFlagFound_b   = FALSE;
            ...
            case FEE_LL_ERASE_START_FLAG_PRESENT_E:
            {
                /* Set the flag that erase start flag was found */
                Fee_stSecDet_ps[xCurrSect_u8].eraseStartFlagFound_b   = TRUE;
 ...
 *********************************************************************
 */
Fee_stRetVal_ten Fee_LLFindEmptyPage(uint8 PhySectIdxUsedSect_u8)
{
    ...
    /* Big and small steps */
    const uint32 smallStep_cu32 = FEE_PAGE_LEN;
    const uint32 bigStep_cu32   = smallStep_cu32 * 16uL;

    static uint32 adrCurr_u32  = 0;    /* Current address which is investigated            */
    static uint32 adrWrite_u32 = 0;    /* New read/write address                           */
    static uint32 adrEnd_u32   = 0;    /* End address - either sector end or big block end */
    static uint32 stepSize_u32 = 0;    /* Small or big step size                           */

    uint32 checkChunk_u32   = 0;                    /* Amount of data for the blank check   */
    Fee_stRetVal_ten xRetVal_en       = FEE_ORDER_PENDING_E;  /* Return value                         */
        ...
        switch(Fee_RdWrOrder_st.Fee_LLFindEmptyPageState_en)
        {
            ...
            case FEE_LL_FINDEMPTYPGE_CHECK_E:
            {
                ...
                else
                {
                    ...
                    if(Fls_BlankCheck(adrCurr_u32, checkChunk_u32) != E_NOT_OK)
                    {
    ...
    Fee_stRetVal_ten xTmpRetVal_en  = FEE_ORDER_PENDING_E;  /* Function temp return */
    Fee_GlobInfoLastRdHeader_tst xBlkHdrTmp_st;             /* Temporary block header */

    #if(FEE_FAST_CACHE_INIT_ACTIVE == FALSE)
    static uint32 xRdAddrStrt_u32;
    uint8 xCacheUpdateType_u8       = FEE_UPDATE_CACHE_IN_INIT; /* Init value */
    #endif

    /* Initialize block header of last read order */
    Fee_LLInitializeLastHdrVariable(&xBlkHdrTmp_st);
 ...
 */
/* MR12 RULE 1505 VIOLATION: This function is used within the same file and has no external linkage */
void Fee_CheckErasedSectorEmpty(void)
{
    uint8  xCurrSect_u8 = 0;        /* Sector loop counter */
    uint8 xCurrSectPhys_u8 = 0;    /* Physical current sector number */
    uint32 xPageAfterErased_u32  = 0;      /* Next page address after erased marker */
    ...
    for(xCurrSect_u8=0; xCurrSect_u8<FEE_NUM_FLASH_BANKS; xCurrSect_u8++)
    {
           ...
           (Fee_LastErasedMarker_u16[xCurrSectPhys_u8] < ((uint16)FEE_NUM_MARKER_IN_HEADER-1u)))
        {
            ...
            xPageAfterErased_u32 = Fee_FlashProp_st[xCurrSectPhys_u8].Fee_PhysStartAddress_u32 +
                                   ((Fee_LastErasedMarker_u16[xCurrSectPhys_u8] + 1u) * FEE_PAGE_LEN);
            ...
            do
            {
                ...
                switch(Fee_RdWrOrder_st.Fee_LLBlankCheckState_en)
                {
                    ...
                    {
                        /* Call the blank check function
                         * Check for complete logical page. */
                        if(Fls_BlankCheck(xPageAfterErased_u32, FEE_PAGE_LEN) != E_NOT_OK)
                        {
 ...
 *********************************************************************
 */
uint8 Fee_GetMostCurrentSectorIdx(void)
{
    uint8 xCntFlashBank_u8;                       /* Loop counter */
    uint8 xActiveSec_u8 = FEE_NO_SEC_IDX_FOUND;   /* Function return */
    uint8 xFirstErasedSec_u8 = FEE_NO_SEC_IDX_FOUND;    /* Temporary function return */
 ...
 *********************************************************************
 */
void Fee_IncAddressInsideSector(uint32* Address_u32, uint16 numBytes_u16, boolean EnsurePageAlign_b)
{
    /* Get physical sector index belonging to the user address */
    uint8 xActSector_u8 = Fee_GetPhysSectorByAddress(*Address_u32);
 ...
 *********************************************************************
 */
uint8 Fee_GetPhysSectorByAddress(uint32 Address_u32)
{
    uint8 cnt_u8;                          /* Loop counter */
    uint8 xSec_u8 = FEE_NO_SEC_IDX_FOUND;  /* Function return */
