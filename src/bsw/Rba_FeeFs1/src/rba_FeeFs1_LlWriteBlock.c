...
#include "Fee_MemMap.h"
Fee_stRetVal_ten Fee_LLWriteBlock(Fee_GlobInfoLastRdHeader_tst* Info_ptr, const uint8* Data_pcu8)
{
    Fee_stRetVal_ten xRetVal_en         = FEE_ORDER_PENDING_E;
    Fee_stRetVal_ten xCompareRetVal_en  = FEE_ORDER_PENDING_E;
    Fee_stRetVal_ten xWrMarkerRetVal_en;
    Fee_stRetVal_ten xErSectorRetVal_en;

    uint16           xNumBytes_u16      = 0;
    uint16           xTempNumBytes_u16  = 0;
    uint32           xNumFreeBytes_u32  = 0;
    uint8            xCnt_u8            = 0;
    uint16           xCnt_u16           = 0;
    uint8            xPhySectIdx_u8     = 0;
    uint8            xLogSectIdx_u8     = 0;
    /* BSW-1076 */
    uint8            xTempPadding       = 0;
    /* END BSW-1076 */

    #if(FEE_ROB_FEATURE_ACTIVE != FALSE)
    uint8       i_u8                    = 0;
    uint16      xNumBytesCutOff_u16     = 0;
    uint8       xCrcOffset_u8           = 0;
    uint8       xNumCrcBytes_u8         = 0;
    uint32      xBlkCrc32Tmp_u32        = 0;
    ...
    switch(Fee_RdWrOrder_st.Fee_LLWrBlock_en)
    {
           ...
           if ((Fee_BlockProperties_st[Fee_OrderFifo_st[Fee_idxActQueue_u8].BlockPropIdx_u16].Flags_u16 & FEE_FLAG_NOFALLBACK_MSK) > 0u)
            {
                ...
                if ((Info_ptr->AdrBlkHeader_u32 != FEE_MAXUINT32) && (Fee_Rb_NoFallback_LastBlockCfg_b != FALSE))
                {
                    ...
                    if( Fls_BlankCheck( (Info_ptr->AdrBlkHeader_u32 - FEE_PAGE_LEN), FEE_PAGE_LEN)!= E_OK)
                    {
                        xRetVal_en = FEE_ABORTED_E;
                    }
                    else
        ...
        case FEE_LL_WR_FILL_SECTOR_END_E:
        {
            ...
            {
                /* Fill the sector end with the complementary erase pattern */
            ...
            else
            {
                ...
                if (xNumFreeBytes_u32 >= FEE_PAGE_LEN)
                {
                    ...
                    if((xNumBytes_u16 % FEE_VIRTUAL_PAGE_SIZE) != 0U)
                    {
                        ...
                        xTempPadding = (uint8)(FEE_VIRTUAL_PAGE_SIZE - (xNumBytes_u16 % FEE_VIRTUAL_PAGE_SIZE));
                    }
...
#define FEE_START_SEC_CODE
#include "Fee_MemMap.h"
Fee_stRetVal_ten Fee_RobIfxHandling(uint8 xActLogWrSect_u8)
{
    Fee_stRetVal_ten xRetVal_en = FEE_ORDER_PENDING_E;
    uint32 i;
