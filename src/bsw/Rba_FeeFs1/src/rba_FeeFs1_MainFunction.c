...
const MemIf_JobResultType Fee_JobTypeMapping[FEE_NUM_RET_VAL_E] =
{
...
#include "Fee_MemMap.h"
boolean Fee_Rb_stRequestStopMode_b;
...
#define FEE_START_SEC_VAR_CLEARED_8
#include "Fee_MemMap.h"
boolean Fee_Prv_stSuspendActiv_b;
boolean Fee_Prv_stHardReOrgErase_b;
#define FEE_STOP_SEC_VAR_CLEARED_8
#include "Fee_MemMap.h"
#endif
...
Fee_Rb_WorkingStateType_ten Fee_Rb_WorkingStateBackUp_en = FEE_RB_IDLE_E;   /* Backup of the working state */
#define FEE_STOP_SEC_VAR_INIT_UNSPECIFIED
#include "Fee_MemMap.h"

#define FEE_START_SEC_VAR_INIT_8
#include "Fee_MemMap.h"
uint8 Fee_idxActQueueBackUp = (uint8)FEE_QUEUE_SIZE;    /* Backup queue index */
...
#include "Fee_MemMap.h"
Fee_DebugMonitoring_tst  Fee_DebugMonitoring;
...
#define FEE_START_SEC_VAR_INIT_8
#include "Fee_MemMap.h"
uint8 Fee_hdrBuffer_au8[FEE_PAGE_LEN] = {0};
#define FEE_STOP_SEC_VAR_INIT_8
#include "Fee_MemMap.h"
#endif
...
void Fee_MainFunction(void)
{
    ...
    Fee_stRetVal_ten xRetVal = FEE_ORDER_PENDING_E;                 /* Order return value */
    boolean stSectReorgInt_b;          /* Indicates an interruptable sector reorganization status */
    boolean  tmpContinueToErase_b = TRUE;
    #if (FEE_RB_ACT_ERASE_SUSPENSION != FALSE)
    Fls_Rb_stSuspend_ten xRetSuspendVal;
    #endif
    ...
    if ((Fee_idxActQueue_u8 < (uint8)FEE_QUEUE_SIZE) && (xRetVal != FEE_ORDER_PENDING_E))
    {
        ...
        if(xRetVal != FEE_ORDER_PENDING_E)
        {
                ...
                (Fee_Rb_WorkingState_en == FEE_RB_INVALIDATE_MODE_E))
            {
                ...
                Fee_Prv_stModuleTest_st.Fee_WriteTimeEnd_u32 = rba_MemLib_GetTimerTicks();

                Fee_Prv_stModuleTest_st.Fee_WriteTimeDiff_u32 = rba_MemLib_ConvertTimerTicksToUs(
                                        Fee_Prv_stModuleTest_st.Fee_WriteTimeEnd_u32 -
                                        Fee_Prv_stModuleTest_st.Fee_WriteTimeStart_u32)/1000u;
            }

            if(Fee_Rb_WorkingState_en == FEE_RB_READ_MODE_E)
            {
                Fee_Prv_stModuleTest_st.Fee_ReadTimeEnd_u32 = rba_MemLib_GetTimerTicks();

                                Fee_Prv_stModuleTest_st.Fee_ReadTimeDiff_u32 = rba_MemLib_ConvertTimerTicksToUs(
                                        Fee_Prv_stModuleTest_st.Fee_ReadTimeEnd_u32 -
                                        Fee_Prv_stModuleTest_st.Fee_ReadTimeStart_u32)/1000u;
            }
            #endif
...
void Fee_LoadNextOrder(void)
{
    ...
    switch(Fee_OrderFifo_st[Fee_idxActQueue_u8].Mode_en)
    {
        ...
        case FEE_FORCED_READ_ORDER:
        {
            ...
            #if (FEE_PRV_DEBUGGING != FALSE)
            Fee_Prv_stModuleTest_st.Fee_ReadTimeStart_u32 = rba_MemLib_GetTimerTicks();
            #endif
        }
        break;
...
{
    uint8 i;
    uint8 idxActQueue_u8;
    uint8 idexHighPrioOrder_u8;
 ...
 */
/* MR12 RULE 1505 VIOLATION: This function is used within the same file and has no external linkage */
void Fee_TriggerHardSectorReorg(Fee_Rb_WorkingStateType_ten WorkingState_en)
{
    /* Save the current working state and queue index in order to come back afterwards */
