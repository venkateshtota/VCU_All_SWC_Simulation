...
uint8 * const Fee_MarkerBufBytePtr_cpu8 = (uint8*)Fee_llMarkerPageBuf_au32;
/* This pointer is used for easy, byte - wise access to the pagebuffer */
/* MR12 RULE 11.3 VIOLATION: Cast necessary for byte wise access */
uint8 * const Fee_DataBytePtr_cpu8 = (uint8*)Fee_llDataBuf_au32;
...
uint8                     Fee_Prv_stInit_u8        = FEE_PRV_ST_INIT_RUNNING; /* Global init state */
uint8                     Fee_Prv_stReorg_u8       = FEE_PRV_ST_REORG_IDLE; /* Global reorganization state */
uint8                     Fee_NumFlashBanksUsed_u8 = FEE_NUM_FLASH_BANKS_AVAILABLE;
...
#include "Fee_MemMap.h"
uint32                  Fee_SecChngCnt_u32 = 0;             /* Fee sector change counter */
...
Fee_RdWrOrder_tst Fee_RdWrOrder_st;

/* Structure contains sector order (0 is eldest, valid sector) */
Fee_LLSectorOrder_tst Fee_LLSectorOrder_st[FEE_NUM_FLASH_BANKS_AVAILABLE];

/* Information regarding the last block header that was read */
Fee_GlobInfoLastRdHeader_tst Fee_GlobInfoLastRdHeader_st;
Fee_LLEraseOrderType_tst  Fee_LLEraseOrder_st;          /* Global sector erase structure */
Fee_LLWrEraseStartFlagOrderType_tst     Fee_LLWrEraseStartFlag_st; /* state machine for writing of Erase Start Flag*/
Fee_OrderFifo_tst       Fee_OrderFifo_st[FEE_QUEUE_SIZE];  /* Fee order queue */
Fee_LLSecReorgStruct_tst  Fee_LLSecReorgStruct_st;        /* Global sector reorganization structure */
Fee_GlobInfoWrBlock_tst   Fee_GlobInfoWrBlock_st;         /* Global sector write structure */
MemIf_JobResultType     Fee_JobResult[FEE_QUEUE_SIZE];                  /* Fee job result */
...
uint8 Fee_idxLLSectorOrder_au8[FEE_NUM_FLASH_BANKS_AVAILABLE];

/* Array containing the sectors that have already been checked completely */
uint8 Fee_CacheUpdCompForSect_au8[FEE_NUM_FLASH_BANKS_AVAILABLE];
uint8                     Fee_idxActQueue_u8;             /* Fee queue index */
uint8                   Fee_RdWrRetries_u8;             /* Read and write retry amounts */
...
#define FEE_START_SEC_VAR_CLEARED_32
#include "Fee_MemMap.h"
uint32 Fee_UnknownBlkCache_Addr_au32[FEE_RB_ACT_UNKNOWN_BLK_CACHE_ARRAY_SIZE];   /* Address of unknown block. */
#define FEE_STOP_SEC_VAR_CLEARED_32
#include "Fee_MemMap.h"

#define FEE_START_SEC_VAR_CLEARED_16
#include "Fee_MemMap.h"
uint16 Fee_UnknownBlkCache_FeeIndex_u16[FEE_RB_ACT_UNKNOWN_BLK_CACHE_ARRAY_SIZE]; /* Index of unknown block. */
...
#include "Fee_MemMap.h"
MemIf_StatusType            Fee_GlobModuleState_st   = MEMIF_UNINIT; /* Global module state */
Fee_stMainType              Fee_stMain               = FEE_POLLING_MODE_E;                     /* Fee main state */
Fee_Rb_WorkingStateType_ten Fee_Rb_WorkingState_en = FEE_RB_IDLE_E;       /* Working state of Fee_MainFunction */
...
#define FEE_START_SEC_VAR_CLEARED_32
#include "Fee_MemMap.h"
uint32                   Fee_DataByteStartCrc_u32;       /* Start value of the robustness checksum */
#define FEE_STOP_SEC_VAR_CLEARED_32
#include "Fee_MemMap.h"
#endif
...
#define FEE_START_SEC_VAR_CLEARED_32
#include "Fee_MemMap.h"
uint32                  Fee_Rb_NoFallback_RemAdr_u32;           /* Address of 2nd copy for double storage */
#define FEE_STOP_SEC_VAR_CLEARED_32
#include "Fee_MemMap.h"

#define FEE_START_SEC_VAR_CLEARED_8
#include "Fee_MemMap.h"
boolean                 Fee_Rb_NoFallback_LastBlockCfg_b;       /* NoFallback configuration of the last found block */
boolean                 Fee_Rb_InvalidateOldCopy_b;             /* Indication to invalidate an old copy */
boolean                 Fee_Rb_MaintainRun_b;                   /* Indicate that Maintain is running */
#define FEE_STOP_SEC_VAR_CLEARED_8
#include "Fee_MemMap.h"
...
const uint8 Fee_FillSectorEndPattern[FEE_PAGES_USED_BY_BLK_HEADER * FEE_PAGE_LEN] =
{
        (uint8)~FEE_ERASE_PATTERN, (uint8)~FEE_ERASE_PATTERN,
        (uint8)~FEE_ERASE_PATTERN, (uint8)~FEE_ERASE_PATTERN,
        (uint8)~FEE_ERASE_PATTERN, (uint8)~FEE_ERASE_PATTERN,
        (uint8)~FEE_ERASE_PATTERN, (uint8)~FEE_ERASE_PATTERN,
        (uint8)~FEE_ERASE_PATTERN, (uint8)~FEE_ERASE_PATTERN,
...
#define FEE_START_SEC_VAR_CLEARED_UNSPECIFIED
#include "Fee_MemMap.h"
Fee_Prv_stModuleTest_tst Fee_Prv_stModuleTest_st;
#define FEE_STOP_SEC_VAR_CLEARED_UNSPECIFIED
#include "Fee_MemMap.h"
#endif
...
#define FEE_START_SEC_VAR_INIT_8
#include "Fee_MemMap.h"
uint8 Fee_hdr2Buffer_au8[FEE_PAGE_LEN] = {0};
#define FEE_STOP_SEC_VAR_INIT_8
#include "Fee_MemMap.h"
#endif
...
#endif
{
    ...
    uint8 local_loop_u8;       /* Local loop variable */
    ...
    #if (FEE_PRV_DEBUGGING != FALSE)
    /* Init max timings */
    uint32 xIniTimeStart_u32 = rba_MemLib_GetTimerTicks();
    uint32 xIniTimeEnd_u32 = 0;
    uint32 xIniTimeCacheStart_u32 = 0;
    uint32 xFindEmptyPageStart_u32 = 0;
    ...
    /* MR12 RULE 11.4 VIOLATION: cast of address required */
    ...
    #if (FEE_PRV_DEBUGGING != FALSE)
    xIniTimeCacheStart_u32 = rba_MemLib_GetTimerTicks();
    #endif
    ...
    Fee_Prv_stModuleTest_st.Fee_CacheInitTime_u32 =
            rba_MemLib_ConvertTimerTicksToUs(rba_MemLib_GetTimerTicks() - xIniTimeCacheStart_u32);
    #endif
    #endif
    ...
    if(Fee_LLSectorOrder_st[xLogSecIdx_u8].SecState_en == FEE_SECTOR_USED_E)
    {
        ...
        #if (FEE_PRV_DEBUGGING != FALSE)
        xFindEmptyPageStart_u32 = rba_MemLib_GetTimerTicks();
        #endif
        ...
        #if (FEE_PRV_DEBUGGING != FALSE)
        Fee_Prv_stModuleTest_st.Fee_FindEmptyPageTime_u32 = rba_MemLib_ConvertTimerTicksToUs(rba_MemLib_GetTimerTicks() - xFindEmptyPageStart_u32);
        #endif
    ...
    #if (FEE_PRV_DEBUGGING != FALSE)
    xIniTimeEnd_u32 = rba_MemLib_GetTimerTicks();

    Fee_Prv_stModuleTest_st.Fee_InitTimeDiff_u32 =
            rba_MemLib_ConvertTimerTicksToUs(xIniTimeEnd_u32 - xIniTimeStart_u32);
...
void Fee_InitVarAndState(void)
{
    uint8 i_u8;                                         /* loop counter */
    ...
    Fee_LLInitializeLastHdrVariable(&Fee_GlobInfoLastRdHeader_st);
 ...
 */
/* MR12 RULE 1505 VIOLATION: This function is used within the same file and has no external linkage */
void Fee_InitCache(void)
{
    uint16 i_u16;               /* loop counter */
 ...
 */
/* MR12 RULE 1505 VIOLATION: This function is used within the same file and has no external linkage */
void Fee_InitOrderFifoBuffer(void)
{
    uint16 i_u16;               /* loop counter */
