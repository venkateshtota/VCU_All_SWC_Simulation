...
#include "Fee_MemMap.h"
Fee_stRetVal_ten Fee_HLMaintainBlock(void)
{
    ...
    do
    {
        ...
        switch (Fee_RdWrOrder_st.Fee_HLMtBlock_en)
        {
            ...
            case FEE_HL_RDWR_BLK_INIT_E:
            {
                ...
                Fee_GlobInfoLastRdHeader_st.BlkCrc32_u32     = 0u;
                Fee_GlobInfoLastRdHeader_st.HdrCrc16_u16     = 0u;
                Fee_GlobInfoLastRdHeader_st.DataBytes_au8[0u] = 0u;
                Fee_GlobInfoLastRdHeader_st.DataBytes_au8[1u] = 0u;
                ...
                switch (xTmpRetVal_en)
                {
                    ...
                    case FEE_ORDER_FINISHED_E:
                    {
                        if (Fee_Rb_GlobSecurityLevelInfo_b == FALSE)
                        {
                            ...
                            Fee_GlobInfoLastRdHeaderMt_st = Fee_GlobInfoLastRdHeader_st;
                            ...
                            #endif
                            {
                                ...
                                while ((xBlockLength_u16 % (uint16)FEE_PAGE_LEN) != 0)
                                {
                                 ...
                                 /* BSW-9817 */ /* [$DD_BSWCODE 19069] */
                                 /* Get start address of the sector containing the block found */
                                 secStartAddr_u32 = Fee_LLGetSecStartAddress(Fee_GetPhysSectorByAddress(Fee_GlobInfoLastRdHeaderMt_st.AdrBlkHeader_u32));

                                /* Check if the redundant block copy is on the same sector of the found block or not
                                 * Note: - The found block will be stored at the address larger than the address of its redundant block copy.
                                 *       - The subtraction (xAdrLastBlkHeader_u32 - xBlockLength_u16) is used to find the start address of its redundant block copy.
                                 *       - In case of relative addressing scheme (1st sector address starts from 0), for some first blocks saved in data flash,
                                 *       the address of block found is smaller than its block length. This leads to illegal address resulting from the subtraction of
                                 *       (xAdrLastBlkHeader_u32 - xBlockLength_u16).
                                 */

                                if((xAdrLastBlkHeader_u32 <= xBlockLength_u16) || ((xAdrLastBlkHeader_u32 - xBlockLength_u16) < secStartAddr_u32))
                                /* END BSW-9817 */
                                {
            ...
            case FEE_HL_CHECK_BLK_HDR_E:
            {
                    ...
                    (Fee_PageBytePtr_cpu8[0] == FEE_BLK_HEADER_PREAMBEL_BYTE_0))
                {
                       ...
                       (xTmpBlkHdr_st.FeeIndex_u16 == Fee_BlockProperties_st[Fee_OrderFifo_st[Fee_idxActQueue_u8].BlockPropIdx_u16].BlockPersistentId_u16))
                   {
                      ...
                      Fee_GlobInfoLastRdHeader_st.DataBytes_au8[0u] = xTmpBlkHdr_st.DataBytes_au8[0u];
                      Fee_GlobInfoLastRdHeader_st.DataBytes_au8[1u] = xTmpBlkHdr_st.DataBytes_au8[1u];
