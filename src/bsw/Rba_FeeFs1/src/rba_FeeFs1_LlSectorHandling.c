...
# if((defined (FEE_RB_GET_NR_FREE_BYTES) && (FALSE!= FEE_RB_GET_NR_FREE_BYTES)) || (FEE_PRV_FREEBYTES_INDFLASH ==TRUE))
static uint32 rba_FeeFs1_getFreeSpace(void);
#endif

# if(defined (FEE_RB_GET_NR_FREE_BYTES) && (FALSE!= FEE_RB_GET_NR_FREE_BYTES))
/**
*********************************************************************
* Fee_Rb_GetNrFreeBytes
...
*********************************************************************
**/
uint32 Fee_Rb_GetNrFreeBytes(void)
{
    uint32 nrFreeBytes_u32;

    if(E_OK == Fee_CheckModuleSt(FEE_SID_RB_GET_NR_FREE_BYTES))
    {
        nrFreeBytes_u32 = rba_FeeFs1_getFreeSpace();
    }
    else
    {
        nrFreeBytes_u32 = 0u;
...
{
    Fee_stRetVal_ten xRetVal_en = FEE_SECTORFULL_E;     /* Function return */
    uint8 xLogSecIdx_u8;                                /* Logical sector index */
    uint8 xPhySecIdx_u8;                                /* Physical sector index */
    uint32 xNumFreeBytes_u32;                            /* Amount of free bytes in the current sector */
    ...
    Fee_FreeByte_thr_st.Fee_Totalfree_bytes=rba_FeeFs1_getFreeSpace();
...
*********************************************************************
*/
# if((defined (FEE_RB_GET_NR_FREE_BYTES) && (FALSE!= FEE_RB_GET_NR_FREE_BYTES)) || (TRUE == FEE_PRV_FREEBYTES_INDFLASH))
...
Fee_stRetVal_ten Fee_LLSectorReorganization(boolean* SectReorgInterSt_pb)
{
    ...
    Fee_stRetVal_ten xTmpRetVal_en  = FEE_ORDER_PENDING_E;  /* Temp return */
    uint8 xLogSecIdx_u8;                                    /* Logical sector index */
    uint8 xPhySecIdx_u8;                                    /* Physical sector index */
    uint32 xTmpAddr_u32 = FEE_CACHE_INIT_VALUE;              /* Temporary block address */
    uint8 xStCache_u8   = 0;                                /* Cache status */
    boolean xSectReorgInterSt_b = FALSE;                    /* Interruptability of the reorg */
    ...
    Fee_BlkHeader_tst xCurrBlkHeader_st;                    /* Block header structure to extract the header details of unknown block */
    ...
    *SectReorgInterSt_pb = FALSE;
    ...
    switch(Fee_RdWrOrder_st.Fee_LLSecReorg_en)
    {
        ...
        case FEE_LL_REORG_INIT_E:
        {
            ...
            #if (FEE_PRV_DEBUGGING != FALSE)
            Fee_Prv_stModuleTest_st.Fee_ReorgTimeStart_u32 = rba_MemLib_GetTimerTicks();
        ...
        case FEE_LL_REORG_PREP_SEARCH_BLK_E:
        {
            ...
            Fee_GlobInfoRedundantRdHeader_st.DataBytes_au8[0u]= 0;                   /* data bytes in the block header */
            Fee_GlobInfoRedundantRdHeader_st.DataBytes_au8[1u]= 0;                   /* data bytes in the block header */
        ...
        case FEE_LL_REORG_SEARCH_UNK_HDRBLK_READ_FINISHED_E:
        {
            ...
            Fee_GlobInfoLastRdHeader_st.DataBytes_au8[0u]  = xCurrBlkHeader_st.DataBytes_au8[0u]; /* data bytes in the header */
            Fee_GlobInfoLastRdHeader_st.DataBytes_au8[1u]  = xCurrBlkHeader_st.DataBytes_au8[1u]; /* data bytes in the header */
    ...
    *SectReorgInterSt_pb = xSectReorgInterSt_b;
    ...
    if(xRetVal_en != FEE_ORDER_PENDING_E)
    {
        ...
        #if (FEE_PRV_DEBUGGING != FALSE)
        Fee_Prv_stModuleTest_st.Fee_ReorgTimeEnd_u32 = rba_MemLib_GetTimerTicks();

        /* MISRA RULE 10.8 VIOLATION: Below type cast is ok as this is only for testing purpose to measure the time. */
        Fee_Prv_stModuleTest_st.Fee_ReorgTimeDiff_u32 = (uint32)rba_MemLib_ConvertTimerTicksToUs(
                Fee_Prv_stModuleTest_st.Fee_ReorgTimeEnd_u32 -
                Fee_Prv_stModuleTest_st.Fee_ReorgTimeStart_u32)/1000u;
...
static Fee_stRetVal_ten Fee_LLRedundantCpyChk(Fee_GlobInfoLastRdHeader_tst * xFee_GlobInfoRedundantRdHeader_pst)
{
    ...
    Fee_BlkHeader_tst   xTmpBlkHdr_st;                                  /* Intermediate variable to store current header under investigation */
    uint32              xExpectedAdrLastBlkHdr_u32;                     /* Expected address of second block instance */
    uint32              len_u32;                                        /* Length of the block to be checked */
    uint16              xCalcCrc_u16;                                   /* CRC of block header */
    ...
    switch (Fee_RdWrOrder_st.Fee_LLRedundantCpyChk_en)
    {
        ...
        case FEE_LL_REDUNDANT_CPY_CHK_INIT_E:
        {
            ...
            len_u32 = xFee_GlobInfoRedundantRdHeader_pst->BlkLength_u16 + FEE_BLOCK_OVERHEAD;
            while(0u != (len_u32 % FEE_PAGE_LEN))
            {
                len_u32++;
            }

            /* Expected location for second instance directly before first instance */
            xExpectedAdrLastBlkHdr_u32 = xFee_GlobInfoRedundantRdHeader_pst->AdrBlkHeader_u32 - len_u32;

            /* In case this is in another sector or a no fallback block search manually (that is the "else" path) */
            /* Note: As the address xAdrFirstBlkHeader_u32 is always within the valid range Fee_GetPhysSectorByAddress never returns 255 */
            if(
                (xExpectedAdrLastBlkHdr_u32 >= Fee_LLGetSecStartAddress(Fee_GetPhysSectorByAddress(xAdrFirstBlkHeader_u32))) &&
                ((xFee_GlobInfoRedundantRdHeader_pst->BlkStatus_u8 & FEE_FLAG_NOFALLBACK_MSK) == 0u)
            )
            {
                /* Directly access stored location to fetch the header */
                if(E_OK == Fls_Read(xExpectedAdrLastBlkHdr_u32, (uint8*)&Fee_PageBytePtr_cpu8[0], FEE_BLOCK_OVERHEAD))
                {
        ...
        /* Wait for Fls to directly deliver second header */
        ...
        /* Perform blank check to exluced ghost headers being evaluated */
        case FEE_LL_REDUNDANT_CPY_CHK_FLS_BLANKCHECK_START_E:
        {
            if(
                E_OK == Fls_BlankCheck(
                    xAdrLastBlkHeader_u32 + ((FEE_PAGES_USED_BY_BLK_HEADER * FEE_PAGE_LEN) - FEE_PHY_PAGE_LEN),
                    FEE_PHY_PAGE_LEN
                )
            )
            {
                Fee_RdWrOrder_st.Fee_LLRedundantCpyChk_en = FEE_LL_REDUNDANT_CPY_CHK_FLS_BLANKCHECK_WAIT_E;
            }
            else
            {
                Fee_RdWrOrder_st.Fee_LLRedundantCpyChk_en = FEE_LL_REDUNDANT_CPY_CHK_SEARCH_HDR_INIT_E;
        ...
        /* Wait for Fls to directly deliver second header */
        case FEE_LL_REDUNDANT_CPY_CHK_FLS_BLANKCHECK_WAIT_E:
        {
            if (Fee_stMain == FEE_POLLING_MODE_E)
            {
                /* Call the Fls main function manually */
        ...
        /* Read successfully delivered some data */
        case FEE_LL_REDUNDANT_CPY_CHK_FLS_READ_SUC_E:
        {
                ...
                (Fee_PageBytePtr_cpu8[0] == FEE_BLK_HEADER_PREAMBEL_BYTE_0))
            {
                ...
                if(
                    ...
                    (0u            == (xTmpBlkHdr_st.BlkStatus_u8 & FEE_MASK_BIT7))
                )
                {
        ...
        {
            /* Error on reading - proceed manually searching */
            Fee_RdWrOrder_st.Fee_LLRedundantCpyChk_en = FEE_LL_REDUNDANT_CPY_CHK_SEARCH_HDR_INIT_E;
        }
        break;

        /* Initialize search of redundant copy */
