...
/**
 *********************************************************************
 * Fee_LLInitializeLastHdrVariable(): The function is used to initialize the variable of type Fee_GlobInfoLastRdHeader_tst
 * that is used in multiple different functions.
 *
 * \param    xLastHdr_ptr:        Pointer to the variable that has to be initialized
 * \return
 * \seealso
 * \usedresources
 *********************************************************************
...
void Fee_LLInitializeLastHdrVariable(Fee_GlobInfoLastRdHeader_tst* xLastHdr_ptr)
{
    xLastHdr_ptr->AdrBlkHeader_u32 = FEE_NULL;            /* Block header address */
    xLastHdr_ptr->BlkLength_u16    = 0;                   /* Block length */
    xLastHdr_ptr->BlkStatus_u8     = 0;                   /* Block status */
    xLastHdr_ptr->FeeIndex_u16     = 0;                   /* Fee index */
    xLastHdr_ptr->HdrCrc16_u16     = 0;                   /* Crc16 checksum (block header) */
    xLastHdr_ptr->BlkCrc32_u32     = 0;                   /* Crc32 checksum (block data) */
    xLastHdr_ptr->DataBytes_au8[0u]= 0;                   /* data bytes in the block header */
    xLastHdr_ptr->DataBytes_au8[1u]= 0;                   /* data bytes in the block header */
}
/**
 *********************************************************************
 ...
 *********************************************************************
 */
Fee_stRetVal_ten Fee_LLSearchSpecifiedBlkHeader(uint16 FeeIdx_u16,
                                                ...
                                                boolean SearchRetry_b)
{
    ...
    Fee_stRetVal_ten xtmpRetVal = FEE_ORDER_PENDING_E;  /* Temp return */

    Fee_GlobInfoLastRdHeader_tst xTmpBlkHdr_st;         /* Temporary block header */
    uint8   xLogSecIdx_u8;                              /* Index of the logical sector */
    boolean xContinueSearch_b = FALSE;                  /* Seach indicator */
    uint32  xTmpAddr_u32 = FEE_CACHE_INIT_VALUE;        /* Temp variable */
    boolean xForcePageBufReload_b = FALSE;              /* Force no reload of the page buffer */
    ...
    Fee_LLInitializeLastHdrVariable(&xTmpBlkHdr_st);
                ...
                case FEE_ORDER_FINISHED_E:
                {
                    ...
                    if((xTmpBlkHdr_st.FeeIndex_u16 == FeeIdx_u16) &&
                       ((Fee_RdWrOrder_st.Fee_LLSecReorg_en == FEE_LL_REORG_SEARCH_BLK_E) ||
                        (Fee_RdWrOrder_st.Fee_LLSecReorg_en == FEE_LL_REORG_REDUNDANT_BLK_CHK_E)||   /* sector change is active and no entry in the Fee_OrderFifo_st exists */
                        ...
                        (Fee_BlockProperties_st[Fee_OrderFifo_st[Fee_idxActQueue_u8].BlockPropIdx_u16].Length_u16 == (xTmpBlkHdr_st.BlkLength_u16 + 4))))
                    {
                        ...
                        if(*LastHdrAddr_ptr > xTmpBlkHdr_st.AdrBlkHeader_u32)
                        {
                            ...
                            GlobBlkHdr_ptr->DataBytes_au8[0u]= xTmpBlkHdr_st.DataBytes_au8[0u];
                            GlobBlkHdr_ptr->DataBytes_au8[1u]= xTmpBlkHdr_st.DataBytes_au8[1u];
 ...
 *********************************************************************
 */
Fee_stRetVal_ten Fee_LLSearchUnknownBlkHeader(uint32* StartHdrAddr_ptr,
                                              uint32* LastHdrAddr_ptr,
                                              Fee_GlobInfoLastRdHeader_tst* GlobBlkHdr_ptr,
                                              boolean* SearchRetry_ptr)
{
    Fee_stRetVal_ten xRetVal    = FEE_ORDER_PENDING_E;  /* Function return */
    Fee_stRetVal_ten xtmpRetVal = FEE_ORDER_PENDING_E;  /* Temp return */

    Fee_GlobInfoLastRdHeader_tst xTmpNxtBlkHdr_st;      /* Temporary block header */

    boolean xForcePageBufReload_b = FALSE;              /* Force no reload of the page buffer */
    uint16  xDummyCacheIdx_u16 = 0;                     /* Cache index dummy */
    uint8   xPhySecIdx_u8  = FEE_NO_SEC_IDX_FOUND;      /* Index of the active sector */
    static uint16           xFeeIdx_u16 = FEE_MAXUINT16;                /* Block index */


    /* Initialize block header of last read order */
    Fee_LLInitializeLastHdrVariable(&xTmpNxtBlkHdr_st);
                    ...
                    case FEE_ORDER_FINISHED_E:
                    {
                        ...
                        if(Fee_SrvBinarySearchInBlockProp(xTmpNxtBlkHdr_st.FeeIndex_u16, &xDummyCacheIdx_u16) == FALSE)
                        {
                               ...
                               (FEE_RB_ACT_UNKNOWN_BLOCK_REORG != FALSE))
                            {
                                ...
                                GlobBlkHdr_ptr->DataBytes_au8[0u]= xTmpNxtBlkHdr_st.DataBytes_au8[0u];
                                GlobBlkHdr_ptr->DataBytes_au8[1u]= xTmpNxtBlkHdr_st.DataBytes_au8[1u];
 ...
 *********************************************************************************************************
 */
Fee_stRetVal_ten Fee_BuildUpCacheUnknownBlk(void)
{
    Fee_stRetVal_ten xRetVal    = FEE_ORDER_PENDING_E;  /* Function return value */
    Fee_stRetVal_ten xtmpRetVal = FEE_ORDER_PENDING_E;  /* To hold return values of the functions Fee_LLCalcBlkCrcInFlash and Fee_LLSearchNextBlkHeader */
    Fee_GlobInfoLastRdHeader_tst xTmpNxtBlkHdr_st;      /* To hold the details of the unkown block header */
    uint16 i;                                           /* For loop Variable */
    uint16 xDummyCacheIdx_u16 = 0;                     /* Dummy cache index for the function Fee_SrvBinarySearchInBlockProp */
    uint8 xPhySecIdx_u8;                              /* Index of the physical sector */
    uint8 xLogSecIdx_u8;                              /* Index of the logical sector */
    boolean xContinueSearch_b = FALSE;                  /* Flag to indicate if search needs to be continued in other sector or not */
    boolean xForcePageBufReload_b = FALSE;              /* Force no reload of the page buffer */
    boolean    xOldUnknonwBlk;                                /* Flag to indicate if the unknown block is new or old */
            ...
            switch(xtmpRetVal)
            {
                ...
                case FEE_ORDER_FINISHED_E:
                {
                    ...
                    if(Fee_SrvBinarySearchInBlockProp(xTmpNxtBlkHdr_st.FeeIndex_u16, &xDummyCacheIdx_u16) == FALSE)
                    {
                        ...
                        /* Save the block info */
                        Fee_GlobInfoLastRdHeader_st = xTmpNxtBlkHdr_st;
...
Fee_stRetVal_ten Fee_LLSearchNextBlkHeader(Fee_GlobInfoLastRdHeader_tst* Fee_GlobInfoLastRdHeader_ptr,
                                           ...
                                           uint32 strAddrHdrSearch)
{
    ...
    uint8           xPhySecIdx_u8;                      /* Physical sector index */
    uint8           xPhyWrSecIdx_u8;                    /* Physical sector index for the next write request */
    uint16          i_u16 = 0;                          /* Loop counter */
    uint16          xCalcCrc_u16 = 0;                   /* Block header CRC */
    uint32          xLoopInRamCounter_u32 = 0;          /* Loop counter */
    ...
    static uint8 noFBPatternBfr_au8[FEE_PAGE_LEN]; /* buffer for reading the content of the noFallBack marker page */
#endif
        ...
        switch(Fee_RdWrOrder_st.Fee_LLRdState_en)
        {
            ...
            case FEE_LL_INIT_READ_E:
            {
                ...
                else
                {
                    ...
                    {
                        /* Cached access is possible, only read the necessary amount of data (only header pages) */
                        xNumBytes2Read_u16 = FEE_VIRTUAL_PAGE_SIZE*FEE_PAGES_USED_BY_BLK_HEADER;
                    }
                    else
                    {
            ...
            case FEE_LL_READ_FINISHED_E:
            {
                ...
                #if (FEE_PRV_DEBUGGING != FALSE)
                xTimeStart_u32 = rba_MemLib_GetTimerTicks();
                #endif
                ...
                /* MR12 RULE 15.4 VIOLATION: Using 2 breaks has no side effects, as the first break is reached when valid header is found. and second when cache is pointing to incorrect location. In both cases, the scan of RAM buffer is completed and software can go to next step. */
                        ...
                        (xCurrBlkHeader_st.Preamble_au8[0] == (uint8)FEE_BLK_HEADER_PREAMBEL_BYTE_0))
                    {
                        ...
                        if((xCurrBlkHeader_st.BlkStatus_u8 & FEE_MASK_BIT7) == 0u)
                        {
                            ...
                            if(xCalcCrc_u16 == xCurrBlkHeader_st.HdrCrc16_u16)
                            {
                                ...
                                Fee_GlobInfoLastRdHeader_ptr->DataBytes_au8[0u]= xCurrBlkHeader_st.DataBytes_au8[0u];
                                Fee_GlobInfoLastRdHeader_ptr->DataBytes_au8[1u]= xCurrBlkHeader_st.DataBytes_au8[1u];
                ...
                Fee_Prv_stModuleTest_st.Fee_SearchNextBlockHeaderLoopRamDiff_u32 +=
                        rba_MemLib_ConvertTimerTicksToUs(rba_MemLib_GetTimerTicks() - xTimeStart_u32);
                #endif
                ...
                else
                {
                    ...
                    if (FastCacheUpdate_u8 != (uint8)FEE_UPDATE_CACHE_IN_INIT)
                    {
                        ...
                        /* MR12 RULE 9.1 VIOLATION: Fee_GlobInfoLastRdHeader_ptr is intialized with values from DFLASH
                         * when the control comes here (same place where the return value is set to FEE_ORDER_FINISHED_E) */
                        if ((Fee_GlobInfoLastRdHeader_ptr->BlkStatus_u8 & (uint8)FEE_FLAG_NOFALLBACK_MSK) > 0)
                        {
                            /* The block shall be considered only if the NoFallback marker is not written
                             -> perform a pattern check of the NoFallBack marker */
                            Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_NOFALLBACK_MARKER_PATTERN_READ_E;
                        }
                        else
                        #endif
                        {
            ...
            #if (FEE_NO_FALLBACK_ACTIVE != FALSE)
            /*************************************************************************************
             * only available if FEE_HEADER_RECHECK_ACTIVE && FEE_NO_FALLBACK_ACTIVE
             * NoFallback can only be activated, if blank check is meaningful (RECHECK_ACTIVE)
             * read the NoFallback marker
             *************************************************************************************/
            case FEE_LL_NOFALLBACK_MARKER_PATTERN_READ_E:
            {
                /* for safety reasons, before reading the NoFallback marker initialize the noFBPatternBfr_au8
                 * to ensure not accessing old data
                 * After read, erase pattern or NoFallback pattern is expected,
                 * so initialize the buffer with the inverse erase pattern */
                Fee_SrvMemSet8(noFBPatternBfr_au8, (uint8)~FEE_ERASE_PATTERN, (uint32)FEE_PAGE_LEN);

                /* It is ensured during write,
                 * that the noFB marker has a valid address and is not overlapping into another sector
                   -> it is not required to check if the address is valid */
                if(E_NOT_OK != Fls_Read(xBlkHeaderAdr_u32 - FEE_PAGE_LEN, noFBPatternBfr_au8, FEE_PAGE_LEN))
                {
                    /* Set next state */
                    Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_NOFALLBACK_MARKER_PATTERN_READ_WAIT_E;
                }
                else
                {
                    /* Fls did not accept the order due to wrong parameters or an internal busy state.
             ...
             * only available if FEE_HEADER_RECHECK_ACTIVE && FEE_NO_FALLBACK_ACTIVE
             * NoFallback can only be activated, if blank check is meaningful (RECHECK_ACTIVE)
             * state to check the noFallBack pattern
             *************************************************************************************/
            case FEE_LL_NOFALLBACK_MARKER_PATTERN_CHECK_E:
            {
                boolean isNoFBPattern_b;
                uint8  const * copyBfr_pcu8;
                uint8  const * noFBPatternBfr_pcu8;
                uint32 i_u32;

                /* compare content of copy bytewise with the noFallBack pattern */
                copyBfr_pcu8 = noFBPatternBfr_au8;
                noFBPatternBfr_pcu8 = Fee_NoFallbackMarkerPattern;
                isNoFBPattern_b = TRUE;

                for(i_u32 = 0uL; (i_u32 < FEE_PAGE_LEN) && (isNoFBPattern_b); i_u32++)
                {
                    if((*copyBfr_pcu8) != (*noFBPatternBfr_pcu8))
                    {
                        isNoFBPattern_b = FALSE;
                    }
                    copyBfr_pcu8++;
                    noFBPatternBfr_pcu8++;
                }
                ...
                if(isNoFBPattern_b)
                {
                    /* noFallBack marker page contains the noFallback pattern */

                    /* perform a blank check over the NoFallBack marker area
                       marker area is blank && noFallback pattern -> pattern is irrelevant, copy is valid
                       marker area is not blank && noFallback pattern -> copy is invalidated */
                    Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_NOFALLBACK_MARKER_BLANK_CHECK_E;
                    xRetVal = FEE_ORDER_PENDING_E;
                }
                else
                {
                    /* noFallBack marker page does not contain the noFallback pattern */
                     /* -> copy is not invalidated and could be used */

                    /* perform next step to check if the block is fully programmed */
                    Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_RECHECK_HEADER_E;
                    xRetVal = FEE_ORDER_PENDING_E;
                }
            }
             ...
             * only available if FEE_HEADER_RECHECK_ACTIVE && FEE_NO_FALLBACK_ACTIVE
             * NoFallback can only be activated, if blank check is meaningful (RECHECK_ACTIVE)
             * blank check NoFallback page
             *************************************************************************************/
            case FEE_LL_NOFALLBACK_MARKER_BLANK_CHECK_E:
            {
                /* It is ensured during write,
                 * that the noFB marker has a valid address and is not overlapping into another sector
                   -> it is not required to check if the address is valid */
                if (Fls_BlankCheck(xBlkHeaderAdr_u32 - FEE_PAGE_LEN, FEE_PAGE_LEN) != E_NOT_OK)
                {
                    /* Set the next state */
                    Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_NOFALLBACK_MARKER_BLANK_CHECK_WAIT_E;

                    /* if NoFallback marker area is blank
                         -> state is switched to FEE_LL_RECHECK_HEADER_E
                       if NoFallback marker area is not blank
                         -> state is switched to FEE_LL_NOFALLBACK_MARKER_BLANK_CHECK_FAILED_E */
                }
                else
                {
                    /* Fls did not accept the order due to wrong parameters or an internal busy state.
            ...
            /*************************************************************************************
             * only available if FEE_HEADER_RECHECK_ACTIVE
             * header blank check
             *************************************************************************************/
            case FEE_LL_RECHECK_HEADER_E:
            {
                /* Call the blank check function */
                if (Fls_BlankCheck(xBlkHeaderAdr_u32 + ((FEE_PAGES_USED_BY_BLK_HEADER * FEE_PAGE_LEN) - FEE_PHY_PAGE_LEN),
            ...
            #if (FEE_NO_FALLBACK_ACTIVE != FALSE)
            case FEE_LL_NOFALLBACK_MARKER_BLANK_CHECK_WAIT_E:
            case FEE_LL_NOFALLBACK_MARKER_PATTERN_READ_WAIT_E:
            #endif
            {
            ...
            #if (FEE_NO_FALLBACK_ACTIVE != FALSE)
            case FEE_LL_NOFALLBACK_MARKER_BLANK_CHECK_FAILED_E:
            #endif
            {
                if (CachedAccess_u8 == (uint8)FEE_USE_MANUAL_ACCESS)
                {
 ...
 *********************************************************************
 */
uint32 Fee_LLGetSecStartAddress(uint8 xPhySectorIdx_u8)
{
 ...
 *********************************************************************
 */
uint32 Fee_LLGetSecEndAddress(uint8 xPhySectorIdx_u8)
{
    /* Access flash property table */
 ...
 *********************************************************************
 */
uint32 Fee_LLGetAddressFromCache(uint16 FeeIdx_u16)
{
    uint16 xCacheIdx_u16;                   /* Cache index */
    uint32 addr_u32 = FEE_CACHE_INIT_VALUE; /* Return value */
 ...
 */
/* MR12 RULE 1505 VIOLATION: This function is used within the same file and has no external linkage */
boolean Fee_LLCheckAddressInCache(uint32 AdrInSector_u32)
{
    boolean xRetVal_b = FALSE;      /* Default return value */
    uint8   xPhySecIdx_u8;          /* Loop counter */
    uint8   xLogSecIdx_u8;          /* Logical sector index */
 ...
 *********************************************************************
 */
void Fee_LLUpdateCacheStForSect(uint8 PhySecIdx_u8)
{
 ...
 *********************************************************************
 */
void Fee_LLEraseCacheStForSect(uint8 PhySecIdx_u8)
{
    /* Update the array indicator for a completed cache update for one sector */
 ...
 *********************************************************************
 */
uint8 Fee_LLGetCacheUpdateStForAllSect(void)
{
    uint8  xFuncRet_u8 = FEE_CACHE_UPD_COMP_FOR_ALL_SECT;
    uint8  xLogSecIdx_u8;               /* Logical sector index */
    uint8  xPhySecIdx_u8;               /* Physical sector index */
 ...
 *********************************************************************
 */
void Fee_LLUpdateAddressInCache(uint16 FeeIdx_u16,
                                                uint32 Addr_u32)
{
    uint16 xCacheIdx_u16;   /* Cache index */
 ...
 */
/* MR12 RULE 1505 VIOLATION: This function is used within the same file and has no external linkage */
uint8 Fee_LLGetCacheUpdateStForSect(uint8 PhySecIdx_u8)
{
 ...
 */
/* MR12 RULE 1505 VIOLATION: This function is used within the same file and has no external linkage */
Fee_stRetVal_ten Fee_BuildUpCache(uint32 StartAdr_u32,
                                                  uint32 EndAdr_u32)
{
    ...
    Fee_stRetVal_ten xRetVal = FEE_ORDER_PENDING_E;  /* Function return */
   #if (FALSE == FEE_HEADER_RECHECK_ACTIVE)  /* Local varaibles are used to save computation time. */
   ...
   uint32 xLastValidHeader_u32 = FEE_NULL;         /* Local variable to store the address of the last valid header. */
        ...
        switch (Fee_RdWrOrder_st.Fee_LLBuildUpCache_en)
        {
            ...
            case FEE_LL_BLD_UP_CACHE_INIT_E:
            {
                   ...
                   (Fee_LLSectorOrder_st[xLogSecIdx_u8].SecState_en == FEE_SECTOR_FULL_E))
                {
                    ...
                    Fee_Prv_stModuleTest_st.BuildUpCacheStart_u32 = rba_MemLib_GetTimerTicks();
                    #endif

                    #if (FALSE == FEE_HEADER_RECHECK_ACTIVE)
            ...
            case FEE_LL_BLD_UP_CACHE_READ_E:
            {
                ...
                {
                    #if  (TRUE == FEE_HEADER_RECHECK_ACTIVE)
...
#if defined(RBA_FLSVECU_FLASH)
                    xDataBuf_au32[0] = rba_FlsVecu_MemRead32_Macro(xRdAdr_u32);
#else
                    /* MISRA RULE 11.4 VIOLATION: Cast is necessary and not critical */
                    /* BSW-1915 */
...
#if defined(RBA_FLSVECU_FLASH)
                        xDataBuf_au32[1] = rba_FlsVecu_MemRead32_Macro(xRdAdr_u32);
#else
                        /* MISRA RULE 11.4 VIOLATION: Cast is necessary and not critical */
                        /* BSW-1915 */ /* [$DD_BSWCODE 976] */
...
#if defined(RBA_FLSVECU_FLASH)
                        xDataBuf_au32[2] = rba_FlsVecu_MemRead32_Macro(xRdAdr_u32);
#else
                        /* MISRA RULE 11.4 VIOLATION: Cast is necessary and not critical */
                        /* BSW-1915 */ /* [$DD_BSWCODE 976] */
...
#if defined(RBA_FLSVECU_FLASH)
                                xDataBuf_au32[1] = rba_FlsVecu_MemRead32_Macro((uint32)(xRdAdr_u32 + 4u));
#else
    ...
    Fee_Prv_stModuleTest_st.BuildUpCacheDiffRemainInside_u32 =
                        rba_MemLib_ConvertTimerTicksToUs(rba_MemLib_GetTimerTicks() -
 ...
 *********************************************************************
 */
Fee_stRetVal_ten Fee_BuildUpCacheForAllSect(void)
{
 ...
 *********************************************************************
 */
Fee_stRetVal_ten Fee_SearchLastBlkHeader(Fee_GlobInfoLastRdHeader_tst* Fee_GlobInfoLastRdHeader_ptr)
{
    Fee_BlkHeader_tst xCurrBlkHeader_st;                            /* Block header structure */
    ...
    do
    {
        ...
        switch (Fee_RdWrOrder_st.Fee_LLRdState_en)
        {
            ...
            case FEE_LL_INIT_READ_E:
            {
                ...
                Fee_GlobInfoLastRdHeader_ptr->DataBytes_au8[0u] = 0;
                Fee_GlobInfoLastRdHeader_ptr->DataBytes_au8[1u] = 0;
...
/* (FALSE == FEE_HEADER_RECHECK_ACTIVE)*/
#endif
 ...
 *********************************************************************
 */
uint32 Fee_SearchHighestCacheEntry(uint32 UpperBoundary_u32, uint8 SectIdx_u8)
{
...
void Fee_InvalidateCacheByAddress(uint32 xAddress_u32)
{
    uint32 i_u32 = 0;                                    /* Loop counter */
