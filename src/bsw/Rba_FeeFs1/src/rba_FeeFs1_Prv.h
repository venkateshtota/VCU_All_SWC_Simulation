...
**********************************************************************************************************************
*/
/* Remap AUTOSAR API to Non-AUTOSAR API, so that the new Fee version could be ported on old Mem version with old Fls,
 * where the AUTOSAR API is not supported. This is required only when AR4.0 version is used. For AR4.2 the AUTOSAR API is supported in Fls. */
#if ((FLS_AR_RELEASE_MAJOR_VERSION == 4u) && (FLS_AR_RELEASE_MINOR_VERSION == 0u))
#define Fls_BlankCheck(TargetAddress,Length) Fls_Rb_BlankCheck(TargetAddress,NULL_PTR,Length)
#endif
...
#define FEE_NUM_MARKER_IN_HEADER    (FEE_NUM_MARKER_E - 2) /* A sector has at least 4 states:
...
enum
{
    ...
    FEE_START_MARKER_ID_E,          /* Contains address of first blockheader */
    ...
    FEE_ERASE_START_FLAG_ID_E,      /* Flag to indicate that erasing of the sector has been started  */
...
typedef struct
{
    ...
    boolean eraseStartFlagFound_b;  /* Flag to indicate if Erase Start Marker area was blank (programmed) or not */
}Fee_stSecDet_tst;
...
typedef enum
{
    ...
    FEE_LL_CPY_BLOCK_DATA_FROM_HDR_E,       /* Read out the data bytes from the header */
...
typedef enum
{
    ...
    #if (FEE_NO_FALLBACK_ACTIVE != FALSE)
    ,FEE_LL_NOFALLBACK_MARKER_PATTERN_READ_E
    ,FEE_LL_NOFALLBACK_MARKER_PATTERN_READ_WAIT_E
    ,FEE_LL_NOFALLBACK_MARKER_PATTERN_CHECK_E
    ,FEE_LL_NOFALLBACK_MARKER_BLANK_CHECK_E
    ,FEE_LL_NOFALLBACK_MARKER_BLANK_CHECK_WAIT_E
    ,FEE_LL_NOFALLBACK_MARKER_BLANK_CHECK_FAILED_E
    #endif
    ...
    #endif
    ,FEE_LL_BLANK_CHECK_ERASE_START_FLAG_E      /* Perform a blank check for the Erase start flag stored in last marker in sector header */
    ,FEE_LL_BLANK_CHECK_ERASE_START_FLAG_WAIT_E /* Wait for the blank check for erase start flag to be finished */
    ,FEE_LL_ERASE_START_FLAG_NOT_PRESENT_E      /* Erase start flag was found (page not blank) in the 12th marker position */
    ,FEE_LL_ERASE_START_FLAG_PRESENT_E          /* No erase start flag was found (page was blank) in the 12th marker position */
...
typedef enum
{
    ...
    FEE_LL_REDUNDANT_CPY_CHK_FLS_READ_WAIT_E,           /* Wait for direct access for second instance to finish */
#if (defined(FEE_HEADER_RECHECK_ACTIVE) && (TRUE == FEE_HEADER_RECHECK_ACTIVE))
    FEE_LL_REDUNDANT_CPY_CHK_FLS_BLANKCHECK_START_E,    /* Trigger blank check on last header page */
    FEE_LL_REDUNDANT_CPY_CHK_FLS_BLANKCHECK_WAIT_E,     /* Wait for blank check on last header page to complete */
#endif
    FEE_LL_REDUNDANT_CPY_CHK_FLS_READ_SUC_E,            /* Successful read of second instance header */
    FEE_LL_REDUNDANT_CPY_CHK_FLS_READ_ERR_E,            /* Failing read of second instance header */
    FEE_LL_REDUNDANT_CPY_CHK_SEARCH_HDR_INIT_E,         /* Initialize manual searching */
...
typedef enum
{
    ...
    #if (FEE_NO_FALLBACK_ACTIVE != FALSE)
    FEE_LL_CPY_FLS2FLS_BLANKCHECK_NOFB_MARKER_E,
    FEE_LL_CPY_FLS2FLS_WAIT_BLANKCHECK_NOFB_MARKER_E,
...
typedef enum
{
    ...
    #endif /* #if  (TRUE == FEE_HEADER_RECHECK_ACTIVE) */
...
typedef struct
{
    ...
    uint8  DataBytes_au8[FEE_FREE_BYTES_AFTER_HEADER]; /* Data bytes in the header */
...
typedef enum
{
    ...
    FEE_ERASESEC_WRITE_ERASESTARTFLAG_E, /* Start erase flag is written from bottom of the sector header */
...
/* State machine for write of erase start flag */
typedef enum
{
    FEE_LLWR_ERASESTARTFLAG_INIT_E = 0,               /* Init state                                    */
    FEE_LLWR_ERASESTARTFLAG_BLK_CHK_E,                /* Perform a blank check over one page           */
    FEE_LLWR_ERASESTARTFLAG_BLK_CHK_WAIT_E,           /* Wait for the blank check order to be finished */
    FEE_LLWR_ERASESTARTFLAG_ALREADY_PROGRAMMED_E,     /* Erase Start Marker already programmed         */
    FEE_LLWR_ERASESTARTFLAG_WRITE_E,                  /* Start write order of the flag                 */
    FEE_LLWR_ERASESTARTFLAG_WRITE_WAIT_E,             /* Wait for the write order to be finished       */
    FEE_LLWR_ERASESTARTFLAG_WRITE_ERROR_E,            /* Write error                                   */
    FEE_LLWR_ERASESTARTFLAG_WRITE_FINISHED_E          /* Flag write was successful (no verification required to keep the flow simple) */
}Fee_LLWrEraseStartFlagStateType_ten;

/* Working variable for writing of Erase Start Flag */
typedef struct
{
    Fee_LLWrEraseStartFlagStateType_ten state_en;               /* State machine state */
    uint32                              eraseStartFlagAddr_u32; /* Address where Erase Start Flag is to be programmed */
...
#include "Fee_MemMap.h"
extern Fee_stRetVal_ten Fee_RobIfxHandling(uint8 xActLogWrSect_u8);
...
/* Assure compatibility to older Mem package versions */
#if (defined(FEE_SW_MAJOR_VERSION) && (FEE_SW_MAJOR_VERSION >= 16))
/* Fee Main type */
typedef enum
{
    FEE_POLLING_MODE_E,
...
extern Fee_LLSectorOrder_tst                Fee_LLSectorOrder_st[FEE_NUM_FLASH_BANKS_AVAILABLE];
...
#if (FEE_PAGES_USED_BY_BLK_HEADER == 2u)    /* Robust block programming states (Hdpg1 -> Data -> Hdpg2 write) */
extern uint8 Fee_hdr2Buffer_au8[FEE_PAGE_LEN];
#endif

/* Workaround Overstamp invalid header to reduce init time */
#if (defined(RBA_FLSIFX_FLASH) || defined(RBA_FLSJDP_FLASH))
extern uint8 Fee_hdrBuffer_au8[FEE_PAGE_LEN];
#endif
/* END Workaround Overstamp invalid header to reduce init time */
...
#include "Fee_MemMap.h"
extern uint8 Fee_idxLLSectorOrder_au8[FEE_NUM_FLASH_BANKS_AVAILABLE];
extern uint8 Fee_idxActQueue_u8;
extern uint8 Fee_CacheUpdCompForSect_au8[FEE_NUM_FLASH_BANKS_AVAILABLE];
extern uint8 Fee_RdWrRetries_u8;

#if (FEE_RB_ACT_ERASE_SUSPENSION != FALSE)
extern boolean Fee_Prv_stSuspendActiv_b;
extern boolean Fee_Prv_stHardReOrgErase_b;
#endif

#if (FEE_RB_ADDONS_CCAS == TRUE)
extern boolean Fee_Rb_stRequestStopMode_b;
#endif
...
#include "Fee_MemMap.h"
extern uint32 Fee_llMarkerPageBuf_au32[FEE_SEC_HEADER_SIZE / 4u];
extern uint32 Fee_Cache_au32[FEE_NUM_BLOCKS];
...
#include "Fee_MemMap.h"
extern uint32 Fee_llPageBuf_au32[(FEE_LL_PAGEBUFFSIZE / 4u) + 2u];
extern uint32 Fee_llDataBuf_au32[FEE_LL_DATABUFFSIZE / 4u];
#define FEE_STOP_SEC_VAR_CLEARED_SECURED_32
#include "Fee_MemMap.h"

#if (defined(FEE_PRV_USE_ROM_TABLE) || ((defined(FEE_RB_USE_ROM_BLOCKTABLE)) && (FEE_RB_USE_ROM_BLOCKTABLE == TRUE)))
#define FEE_START_SEC_CONST_UNSPECIFIED
#include "Fee_MemMap.h"
...
#define FEE_START_SEC_VAR_CLEARED_32
#include "Fee_MemMap.h"
extern uint32   Fee_Rb_NoFallback_RemAdr_u32;
#define FEE_STOP_SEC_VAR_CLEARED_32
#include "Fee_MemMap.h"

#define FEE_START_SEC_VAR_CLEARED_8
#include "Fee_MemMap.h"
extern boolean  Fee_Rb_NoFallback_LastBlockCfg_b;
extern boolean  Fee_Rb_InvalidateOldCopy_b;
extern boolean  Fee_Rb_MaintainRun_b;
#define FEE_STOP_SEC_VAR_CLEARED_8
#include "Fee_MemMap.h"
...
extern const uint8 Fee_FillSectorEndPattern[FEE_PAGES_USED_BY_BLK_HEADER * FEE_PAGE_LEN];
...
#define FEE_START_SEC_VAR_CLEARED_32
#include "Fee_MemMap.h"
extern uint32 Fee_UnknownBlkCache_Addr_au32[FEE_RB_ACT_UNKNOWN_BLK_CACHE_ARRAY_SIZE];   /* Address of unknown block. */
#define FEE_STOP_SEC_VAR_CLEARED_32
#include "Fee_MemMap.h"

#define FEE_START_SEC_VAR_CLEARED_16
#include "Fee_MemMap.h"
extern uint16 Fee_UnknownBlkCache_FeeIndex_u16[FEE_RB_ACT_UNKNOWN_BLK_CACHE_ARRAY_SIZE]; /* Index of unknown block. */
...
#if(FEE_RB_ACT_UNKNOWN_BLOCK_CACHE_SIZE == 0)
extern Fee_stRetVal_ten Fee_LLSearchUnknownBlkHeader(uint32* StartHdrAddr_ptr,
                                                     uint32* LastHdrAddr_ptr,
                                                     Fee_GlobInfoLastRdHeader_tst* GlobBlkHdr_ptr,
                                                     boolean* SearchRetry_ptr);
#else
/* Unknown blcok sector reorganiation with cache */
extern Fee_stRetVal_ten Fee_BuildUpCacheUnknownBlk(void);
...
#if defined (FEE_PRV_ROBUST_SECT_OVERFLOW)
extern Fee_stRetVal_ten Fee_LlRobustSectorOverflow(void);
#endif
...
LOCAL_INLINE void       Fee_LLCopyPageBuff2HeaderStart(Fee_BlkHeader_tst* BlkHdr_pst, const uint8* PageBuf_pcu8);
#if(FEE_FAST_CACHE_INIT_ACTIVE != FALSE)
LOCAL_INLINE void       Fee_LLCopyPageBuff2HeaderIdxAndCrc(Fee_BlkHeader_tst* BlkHdr_pst, const uint8* PageBuf_pcu8);
...
#if(FEE_FAST_CACHE_INIT_ACTIVE != FALSE)
LOCAL_INLINE boolean Fee_SrvBinarySearchInBlockPropFast(uint16 FeeIdx_u16, uint16* CacheIdx_pu16);
...
#if (FEE_PRV_DEBUG_MONITOR != FALSE)
extern void  Fee_DBM_RepError(uint16 FeeBlkIdx, Fee_DebugMonitoringFct_ten xError, uint8 xErrorInfo);
#endif
#define FEE_STOP_SEC_CODE
#include "Fee_MemMap.h"
