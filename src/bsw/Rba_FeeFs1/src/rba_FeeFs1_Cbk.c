...
#define FEE_START_SEC_CODE
#include "Fee_MemMap.h"
void Fee_SwitchJobEndNotification(void)
#endif
{
    ...
    else
    {
        ...
        /* Switch the search sectors state machine */
        if(Fee_RdWrOrder_st.Fee_LLRdState_en == FEE_LL_BLANK_CHECK_ERASE_START_FLAG_WAIT_E)
        {
            /* Page was blank */
            Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_ERASE_START_FLAG_NOT_PRESENT_E;
        }
        ...
        /* Switch the read state machine */
        if(Fee_RdWrOrder_st.Fee_LLRdState_en == FEE_LL_NOFALLBACK_MARKER_PATTERN_READ_WAIT_E)
        {
            /* successful Fls_Read of the noFallBack pattern */
            Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_NOFALLBACK_MARKER_PATTERN_CHECK_E;
        }

        /* Switch state machine of the function Fee_LLSearchNextBlkHeader */
        if(Fee_RdWrOrder_st.Fee_LLRdState_en == FEE_LL_NOFALLBACK_MARKER_BLANK_CHECK_WAIT_E)
        {
            /* Fls_BlankCheck was finished with success */
        ...
        /* Switch state machine of the function Fee_LLCpyBlkFromFls2Fls */
        if(Fee_RdWrOrder_st.Fee_LLCpyBlkFls2Fls_en == FEE_LL_CPY_FLS2FLS_WAIT_BLANKCHECK_NOFB_MARKER_E)
        {
            // page is not programmed, write the no fall back marker
...
/* FEE_HEADER_RECHECK_ACTIVE != FALSE */
        ...
        /* Switch state machine of the function Fee_LLWrEraseStartFlag */
        if(FEE_LLWR_ERASESTARTFLAG_BLK_CHK_WAIT_E == Fee_LLWrEraseStartFlag_st.state_en)
        {
            /* Page is blank -> go to next state to program the Erase Start Flag */
            Fee_LLWrEraseStartFlag_st.state_en = FEE_LLWR_ERASESTARTFLAG_WRITE_E;
        }

        /* Switch state machine of the function Fee_LLWrEraseStartFlag */
        if(FEE_LLWR_ERASESTARTFLAG_WRITE_WAIT_E == Fee_LLWrEraseStartFlag_st.state_en)
        {
            /* Writing of Erase Start Flag was successful -> go to next state to verify the program operation */
            Fee_LLWrEraseStartFlag_st.state_en = FEE_LLWR_ERASESTARTFLAG_WRITE_FINISHED_E;
        }
    ...
    #if  (TRUE == FEE_HEADER_RECHECK_ACTIVE) /* For Renesas controllers */
        ...
        if(Fee_RdWrOrder_st.Fee_LLRedundantCpyChk_en == FEE_LL_REDUNDANT_CPY_CHK_FLS_READ_WAIT_E)
        {
            Fee_RdWrOrder_st.Fee_LLRedundantCpyChk_en = FEE_LL_REDUNDANT_CPY_CHK_FLS_BLANKCHECK_START_E;
        }

        if(Fee_RdWrOrder_st.Fee_LLRedundantCpyChk_en == FEE_LL_REDUNDANT_CPY_CHK_FLS_BLANKCHECK_WAIT_E)
        {
            Fee_RdWrOrder_st.Fee_LLRedundantCpyChk_en = FEE_LL_REDUNDANT_CPY_CHK_FLS_READ_ERR_E;
        }
        ...
        if(Fee_RdWrOrder_st.Fee_LLRedundantCpyChk_en == FEE_LL_REDUNDANT_CPY_CHK_FLS_READ_WAIT_E)
        {
            Fee_RdWrOrder_st.Fee_LLRedundantCpyChk_en = FEE_LL_REDUNDANT_CPY_CHK_FLS_READ_SUC_E;
        }
        #endif
    }
}
...
#define FEE_START_SEC_CODE
#include "Fee_MemMap.h"
void Fee_SwitchJobErrorNotification(void)
#endif
{
    /* Switch the read state machine */
    if(Fee_RdWrOrder_st.Fee_LLRdState_en == FEE_LL_WAIT_READ_PAGE_E)
    ...
    {
        /* Page was not blank */
        Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_ERASE_START_FLAG_PRESENT_E;
    }
    ...
    if(Fee_RdWrOrder_st.Fee_LLRdState_en == FEE_LL_NOFALLBACK_MARKER_PATTERN_READ_WAIT_E)
    {
        /* Fls_BlankCheck was not finished with success */
        Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_READ_ERROR_E;
    }
    ...
    {
        /* Fls_BlankCheck was not finished with success */
        Fee_RdWrOrder_st.Fee_LLRdState_en = FEE_LL_NOFALLBACK_MARKER_BLANK_CHECK_FAILED_E;
    }
    ...
    if(Fee_RdWrOrder_st.Fee_LLCpyBlkFls2Fls_en == FEE_LL_CPY_FLS2FLS_WAIT_BLANKCHECK_NOFB_MARKER_E)
    {
        // page is already programmed, skip writing of no fallback marker and go to next state
        Fee_RdWrOrder_st.Fee_LLCpyBlkFls2Fls_en = FEE_LL_CPY_FLS2FLS_READ_E;
    }
...
/* FEE_HEADER_RECHECK_ACTIVE != FALSE */
    ...
    if(FEE_LLWR_ERASESTARTFLAG_BLK_CHK_WAIT_E == Fee_LLWrEraseStartFlag_st.state_en)
    {
        /* Page is not blank -> Erase Start Marker is already programmed, return finished */
        Fee_LLWrEraseStartFlag_st.state_en = FEE_LLWR_ERASESTARTFLAG_ALREADY_PROGRAMMED_E;
    }

    /* Switch state machine of the function Fee_LLWrEraseStartFlag */
    if(FEE_LLWR_ERASESTARTFLAG_WRITE_WAIT_E == Fee_LLWrEraseStartFlag_st.state_en)
    {
        /* Writing of Erase Start Flag was unsuccessful -> go to next state to first check if something was programmed */
        Fee_LLWrEraseStartFlag_st.state_en = FEE_LLWR_ERASESTARTFLAG_WRITE_ERROR_E;
    }
