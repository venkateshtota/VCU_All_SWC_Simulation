...
static void Fee_LLPrepPageBufWithCompleteHdrData(const Fee_GlobInfoLastRdHeader_tst * Fee_GlobInfoLastRdHeader_pcst);
 ...
 *********************************************************************
 */
Fee_stRetVal_ten Fee_LLCompBlkInFlash(const Fee_GlobInfoLastRdHeader_tst* HeaderInfo_pcst,
                                                      const uint8* Data_pcu8)
{
    Fee_stRetVal_ten xRetVal_en = FEE_ORDER_PENDING_E; /* Function return */
    uint16 xNumBytes_u16  = 0;              /* Number of bytes */
    uint8  xLogSectIdx_u8 = 0;              /* Logical sector index */
    uint8  xPhySectIdx_u8 = 0;              /* Physical sector index */
    uint8  xPhyWrSectIdx_u8 = 0;            /* Physical sector index of the write address */
    uint16 xCmpLen_u16;
 ...
 *********************************************************************
 */
void Fee_LLCopyPageBuff2Marker(Fee_MarkerProp_t* Marker_pst,
                                               const uint8* PageBuf_pcu8)
{
    /* 0xCAFE <- [0]: CA, [1]: FE */
 ...
 *********************************************************************
 */
void Fee_LLPrepMarkerBufWithMarkerData(const Fee_MarkerProp_t* Marker_pcst,
                                                       uint8* MarkerBuf_pu8)
{
    /* 0xCAFE <- [0]: CA, [1]: FE */
 ...
 *          uint16  HdrCrc16_u16;
 *          uint32  BlkCrc32_u32;
 *          uint8   DataBytes_au8[FEE_FREE_BYTES_AFTER_HEADER];
 *       }Fee_BlkHeader_tst;
 *
 ...
 *********************************************************************
 */
void Fee_LLCopyPageBuff2HeaderMid(Fee_BlkHeader_tst * BlkHdr_pst,
                                                  const uint8* PageBuf_pcu8)
{
 ...
 *********************************************************************
 */
void Fee_LLCopyPageBuff2HeaderEnd(Fee_BlkHeader_tst* BlkHdr_pst,
                                                  const uint8* PageBuf_pcu8)
{
    ...
    BlkHdr_pst->DataBytes_au8[0u] = PageBuf_pcu8[14];
    BlkHdr_pst->DataBytes_au8[1u] = PageBuf_pcu8[15];
}
 ...
 *********************************************************************
 */
void Fee_LLPrepPageBufWithHdrDataStart(Fee_GlobInfoLastRdHeader_tst* HeaderInfo_pst)
{
    uint16 xPageCrc_u16 = 0;
 ...
 *********************************************************************
 */
void Fee_LLPrepPageBufWithHdrDataEnd(Fee_GlobInfoLastRdHeader_tst* HeaderInfo_pst,
                                                       const uint8* Data_pcu8,
                                                       uint32 BlkCrc32_u32)
{
    uint16 xNumBytes_u16 = 0;
    uint16 i = 0;
 ...
 *********************************************************************
 */
Fee_stRetVal_ten Fee_LLCopyData2Buffer(const Fee_GlobInfoLastRdHeader_tst* Fee_GlobInfoLastRdHeader_pcst,
                                                       uint8* DataPtr_pu8,
                                                       uint16 DataOffset_u16,
                                                       uint16 DataLength_u16)
{
    Fee_stRetVal_ten xRetVal_en = FEE_ORDER_PENDING_E;     /* Function return */
    uint32 xNumBytesToSectEnd_u32 = 0;        /* Amount of bytes to the end of the sector */
    uint16 xNumBytesOfsConsid_u16 = 0;        /* Amount of bytes that are not relevant */
    uint8  xLogSectIdx_u8         = 0;        /* Logical sector index */
    uint8  xCntSect_u8            = 1;        /* Sector counter */
    uint8  xPhySectIdx_u8         = 0;        /* Physical sector index */
    boolean xStartAdrFound_b      = FALSE;    /* Indicator for exiting the search loop */
    ...
    switch (Fee_RdWrOrder_st.Fee_LLCopyBlk_en)
    {
        ...
        case FEE_LL_CPY_BLK_INIT_E:
        {
            ...
            if(DataOffset_u16 < FEE_FREE_BYTES_AFTER_HEADER)
            {
                /* data bytes from the header have to be read out, read this information directly from the global variable */
                Fee_RdWrOrder_st.Fee_LLCopyBlk_en = FEE_LL_CPY_BLOCK_DATA_FROM_HDR_E;
            }
            else
            {
                /* no data from the header has to be read, go to state to read from Fls */
                Fee_RdWrOrder_st.Fee_LLCopyBlk_en = FEE_LL_CPY_BLOCK_START_E;
            }
        }
        break;

        /* Data bytes from the header have to be read out, read it from the global variable */
        case FEE_LL_CPY_BLOCK_DATA_FROM_HDR_E:
        {
            uint8 i_u8;
            uint16 startOfs_u16;
            uint16 endOfs_u16;
            uint8 nrOfDataBytesToBeRead_u8;

            startOfs_u16 = DataOffset_u16;
            endOfs_u16 = DataOffset_u16 + DataLength_u16;

            /* Compute the number of bytes to be read out from the BC in RAM */
            if(endOfs_u16 > FEE_FREE_BYTES_AFTER_HEADER)
            {
                /* data pages have to be also read out. */
                nrOfDataBytesToBeRead_u8 = (uint8)FEE_FREE_BYTES_AFTER_HEADER - (uint8)DataOffset_u16;
            }
            else
            {
                /* Only data from the header page requested */
                nrOfDataBytesToBeRead_u8 = (uint8)DataLength_u16;
            }

            for(i_u8 = 0u; i_u8 < nrOfDataBytesToBeRead_u8; i_u8++)
            {
                DataPtr_pu8[i_u8] = Fee_GlobInfoLastRdHeader_pcst->DataBytes_au8[startOfs_u16];

                /* Increment the read address for the next Fls access (overflow is checked automatically) */
                Fee_IncAddressInsideSector(&Fee_RdWrOrder_st.xRdAddress, 1u, FALSE);
                Fee_GlobInfoWrBlock_st.BytesAlrdyConsid_u16++;
                Fee_GlobInfoWrBlock_st.Bytes2Read_u16--;
                startOfs_u16++;
            }

            /* Check if the data read from the current sector is complete */
            if(0u != Fee_GlobInfoWrBlock_st.Bytes2Read_u16)
            {
                /* Some more information from current sector have to be read out, use Fls_Read to read this information from DFLASH*/
                Fee_RdWrOrder_st.Fee_LLCopyBlk_en = FEE_LL_CPY_BLOCK_START_E;
            }
            else
            {
                /* Required bytes for this step (from the current sector) is completed. Go to next state to check if there are more bytes to be read out*/
                Fee_RdWrOrder_st.Fee_LLCopyBlk_en = FEE_LL_CPY_BLOCK_FINISHED_E;
            }
        }break;

         /* Read out the data from DFLASH */
        case FEE_LL_CPY_BLOCK_START_E:
        {
            #if (FEE_PRV_DEBUGGING != FALSE)
            Fee_Prv_stModuleTest_st.Fee_DataReadStart_u32 = rba_MemLib_GetTimerTicks();
        ...
        {
            #if (FEE_PRV_DEBUGGING != FALSE)
            Fee_Prv_stModuleTest_st.Fee_DataReadEnd_u32 = rba_MemLib_GetTimerTicks();

            Fee_Prv_stModuleTest_st.Fee_DataReadDiff_u32 = rba_MemLib_ConvertTimerTicksToUs(
...
/**
 *********************************************************************
 * Fee_LLPrepPageBufWithCompleteHdrData(): Prepare the page buffer with complete block header (including first 2 data bytes)
 *
 * Hint: Block header structure:
 *       {
 *          uint8   Preamble_au8[3];
...
static void Fee_LLPrepPageBufWithCompleteHdrData(const Fee_GlobInfoLastRdHeader_tst * Fee_GlobInfoLastRdHeader_pcst)
{
    ...
    Fee_PageBytePtr_cpu8[3] = Fee_GlobInfoLastRdHeader_pcst->BlkStatus_u8;

    /* MSByte: 0xD2E9 -> D2 */
    Fee_PageBytePtr_cpu8[4] = (uint8)(Fee_GlobInfoLastRdHeader_pcst->FeeIndex_u16 >> 8u);
    /* LSByte:        -> E9*/
    Fee_PageBytePtr_cpu8[5] = (uint8)((Fee_GlobInfoLastRdHeader_pcst->FeeIndex_u16) & 0xFFu);

    /* MSByte: 0x08   -> 00 */
    Fee_PageBytePtr_cpu8[6] = (uint8)(Fee_GlobInfoLastRdHeader_pcst->BlkLength_u16 >> 8u);
    /* LSByte:        -> 08 */
    Fee_PageBytePtr_cpu8[7] = (uint8)((Fee_GlobInfoLastRdHeader_pcst->BlkLength_u16) & 0xFFu);

    /* Header Crc */
    Fee_PageBytePtr_cpu8[8] = (uint8)(Fee_GlobInfoLastRdHeader_pcst->HdrCrc16_u16 >> 8u);
    Fee_PageBytePtr_cpu8[9] = (uint8)((Fee_GlobInfoLastRdHeader_pcst->HdrCrc16_u16) & 0xFFu);

    /* Add the block CRC to the page buffer */
    Fee_PageBytePtr_cpu8[10] = (uint8) (Fee_GlobInfoLastRdHeader_pcst->BlkCrc32_u32 >> 24u);
    Fee_PageBytePtr_cpu8[11] = (uint8) (Fee_GlobInfoLastRdHeader_pcst->BlkCrc32_u32 >> 16u);
    Fee_PageBytePtr_cpu8[12] = (uint8) (Fee_GlobInfoLastRdHeader_pcst->BlkCrc32_u32 >> 8u);
    Fee_PageBytePtr_cpu8[13] = (uint8)  Fee_GlobInfoLastRdHeader_pcst->BlkCrc32_u32; // Would be a shift by 0

    /* Add the data bytes in the header */
    Fee_PageBytePtr_cpu8[14] = (uint8)  Fee_GlobInfoLastRdHeader_pcst->DataBytes_au8[0];
    Fee_PageBytePtr_cpu8[15] = (uint8)  Fee_GlobInfoLastRdHeader_pcst->DataBytes_au8[1];
}
 ...
 *********************************************************************
 */
Fee_stRetVal_ten Fee_LLCpyBlkFromFls2Fls(const Fee_GlobInfoLastRdHeader_tst* Fee_GlobInfoLastRdHeader_pcst, boolean Fee_WriteTwice_b)
{
    Fee_stRetVal_ten xRetVal_en = FEE_ORDER_PENDING_E;  /* Function return */
    uint8 xPhySectIdx_u8;                               /* Physical sector index */
    uint8 xLogSectIdx_u8;                                /* Logical sector index */
    uint32 xNumBytes_u32;                               /* Calculated number of bytes (different usage) */
    ...
    Fee_stRetVal_ten xWrMarkerRetVal_en;                /* Return value for writing markers */
    uint16 xCnt_u16;                                    /* Block and sector number counter */
    static uint8 xCntRetry_u8 = FEE_MAX_RD_WR_COMP_RETRIES;             /* Retries for write/read and verify after failed compare */
    uint16 xConfigPropTableIdx_u16;                     /* Configuration properties table index */
    Fee_stRetVal_ten xRetValErase_en;
    ...
    switch (Fee_RdWrOrder_st.Fee_LLCpyBlkFls2Fls_en)
    {
        ...
        case FEE_LL_CPY_FLS2FLS_INIT_E:
        {
            ...
            if ((Fee_GlobInfoLastRdHeader_pcst->BlkStatus_u8 & (uint8)FEE_FLAG_NOFALLBACK_MSK) > 0)
            {
                    ...
                    ((Fee_GlobInfoLastRdHeader_pcst->BlkStatus_u8 & (uint8)FEE_FLAG_SEC_LEVEL_MSK) == FEE_BLK_INT_DOUBLE_STORAGE))
                {
                    ...
                    {
                        Fee_RdWrOrder_st.Fee_LLCpyBlkFls2Fls_en  = FEE_LL_CPY_FLS2FLS_BLANKCHECK_NOFB_MARKER_E;
                    }
                }
            }
            /* #if (FEE_NO_FALLBACK_ACTIVE != FALSE) */
        ...
        #if (FEE_NO_FALLBACK_ACTIVE != FALSE)
        case FEE_LL_CPY_FLS2FLS_BLANKCHECK_NOFB_MARKER_E:
        {
            if ((Fee_Rb_InvalidateOldCopy_b != FALSE) && (Fee_LLSecReorgStruct_st.xCntCopies_u8 == 0))
            {
            ...
            if (Fls_BlankCheck(tmpAddr_u32, FEE_PAGE_LEN) != E_NOT_OK)
            {
                /* Wait for the over programming to finish */
                Fee_RdWrOrder_st.Fee_LLCpyBlkFls2Fls_en = FEE_LL_CPY_FLS2FLS_WAIT_BLANKCHECK_NOFB_MARKER_E;
            }
            else
            {
                /* Return an error due to wrong parameters or a busy/not init state of the Fls */
        ...
        case FEE_LL_CPY_FLS2FLS_WAIT_BLANKCHECK_NOFB_MARKER_E:
        ...
        case FEE_LL_CPY_FLS2FLS_HDRPG1_WRITE_E:
        {
            ...
            /* To have consistent data between validation and writing, the header should be read only once.
             * Re-use the information that was read out during header validation to get the contents of the header.
             * This information is available in the variable Fee_GlobInfoLastRdHeader_pcst. */
            Fee_LLPrepPageBufWithCompleteHdrData(Fee_GlobInfoLastRdHeader_pcst);
        ...
        case FEE_LL_CPY_FLS2FLS_WRITE_E:
        {
             ...
             * Re-use the information that was read out during header validation to get the contents of the header.
             * This information is available in the variable Fee_GlobInfoLastRdHeader_pcst. */
            if(0u == cntProgrammedBytes_u32)
            {
                /* First bytes are being programmed, which means its a header page programming. update the header. */
                Fee_LLPrepPageBufWithCompleteHdrData(Fee_GlobInfoLastRdHeader_pcst);
            }
#endif
            if (Fls_Write(Fee_RdWrOrder_st.xWrAddress,
 ...
 */
#if defined (FEE_PRV_ROBUST_SECT_OVERFLOW)
Fee_stRetVal_ten Fee_LlRobustSectorOverflow(void)
{
    Fee_stRetVal_ten    xRetVal_en      = FEE_ORDER_PENDING_E;  /* Function return */
    Fee_stRetVal_ten    xTmpRetVal_en   = FEE_ORDER_PENDING_E;  /* Temporary return value of llErase Function */
    uint8               xLogSecIdx_u8;                          /* Index of the logical sector */
    uint8               xPhySectIdx_u8;                         /* Index of the physical sector */
    static Fls_LengthType xDataSize_uo;                           /* Size of data to be copied. */
    static Fls_LengthType xCntPgmSize_uo;                         /* Amount of bytes copied in one step. */
    ...
    switch (Fee_RdWrOrder_st.Fee_LLRobustSectOverflow_en)
    {
        ...
        {
            if (Fls_BlankCheck(Fee_RdWrOrder_st.xRdAddress, xCntPgmSize_uo) != E_NOT_OK)
            {
 ...
 *********************************************************************
 */
Fee_stRetVal_ten Fee_LLCalcBlkCrcInFlash(const Fee_GlobInfoLastRdHeader_tst* HeaderInfo_pcst)
{
    Fee_stRetVal_ten xRetVal_en = FEE_ORDER_PENDING_E;  /* Function return */
    uint8 xPhySectIdx_u8 = 0;                           /* Physical sector index */
    uint8 xPhyWrSectIdx_u8 = 0;                         /* Physical sector index of the write address */
    uint8 xLogSectIdx_u8 = 0;                           /* Logical sector index */
    uint16 xDataBlkLength_u16  = 0;                     /* Temporary data length */
    ...
    #if(FEE_ROB_FEATURE_ACTIVE != FALSE)
    uint8 i_u8 = 0;                                     /* Loop counter */
    uint32 xNumBytesLeft_32 = 0;                        /* Number of bytes left for the current sector */
 ...
 *********************************************************************
 */
uint32 Fee_CalculateNumOfFreeBytesInCurSector(uint32 DataEndAdr_u32)
{
    uint8  xPhySecIdx_u8;               /* Physical sector index */
    uint32 xNumFreeBytes_u32;           /* Amount of free bytes */
 ...
 */
#if (STD_ON == FEE_POLLING_MODE)
void Fee_CheckFlsJobResult(void)
{
    MemIf_JobResultType FlsJobResult = MEMIF_JOB_PENDING;
