...
Fee_stRetVal_ten Fee_LLWriteMarker(uint8 PhySectIdx_u8, uint8 MarkerID_u8)
{
    Fee_MarkerProp_t xWrMarker_st;                      /* Marker structure */
    Fee_stRetVal_ten xRetVal_en = FEE_ORDER_PENDING_E;  /* Function return value */
    uint32 xSecChngCntTmp_u32 = 0;                      /* Temporary sector change counter */
    ...
    switch (Fee_RdWrOrder_st.Fee_LLWrMarker_en)
    {
        ...
        case FEE_LL_MARKER_BLK_CHK_E:
        {
            ...
            if(Fls_BlankCheck(xRdWr_Address_u32, FEE_PAGE_LEN) != E_NOT_OK)
            {
        ...
        case FEE_LL_MARKER_BLK_CHK_ERROR_E:
        {
            ...
            if(xRdWr_Address_u32 >
               (Fee_FlashProp_st[PhySectIdx_u8].Fee_PhysStartAddress_u32 + (uint32)(((uint8)FEE_NUM_MARKER_IN_HEADER - 1u) * FEE_PAGE_LEN)))
            {
